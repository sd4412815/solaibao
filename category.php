...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        $sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max ".
               ...
               " WHERE ($children OR " . get_extension_goods($children) . ') AND g.supplier_id=0 AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  AND g.goods_number!=0 ';
               //获得当前分类下商品价格的最大值、最小值
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            ...
            "WHERE g.brand_id = b.brand_id AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) . ") AND g.supplier_id=0 AND b.is_show = 1 " .
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        {
            $sql = "SELECT a.attr_name FROM " . $ecs->table('attribute') . " AS a, " . $ecs->table('goods_attr') . " AS ga, " . $ecs->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value' AND g.goods_number!=0 ";
            if($temp_name = $db->getOne($sql))
            {
                ...
                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       ...
                       " AND a.attr_id='$value' AND g.goods_number!=0 ".
    ...
    $goodslist = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order)['goods_list'];
    $recount = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order)['recount'];
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $where = "g.is_on_sale = 1 AND g.supplier_id=0 AND g.is_alone_sale = 1 AND ".
    ...
    $sql = "SELECT COUNT(*) AS count FROM ". $GLOBALS['ecs']->table('goods') .' AS g ' .'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE g.goods_number > 0 AND $where $ext";
    $recount = $GLOBALS['db']->getRow($sql);
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.goods_number, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE g.goods_number > 0 AND $where $ext ORDER BY $sort $order";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        // $arr[$row['goods_id']]['recount']          = $recount['count'];
    ...
    // p($arr);
    return array('goods_list'=>$arr,'recount'=>$recount['count']);
}
...
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='')
{
    ...
    /* 返回商品总数 */
    return $GLOBALS['db']->getOne('SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . ' AS sgc LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .'ON sgc.cat_id = g.cat_id WHERE  is_on_sale = 1 AND is_delete = 0 AND goods_number > 0  AND supplier_id = 0 AND '.$children." LIMIT ".$size_page."," . $size);
}
