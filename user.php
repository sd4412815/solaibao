...
include_once(ROOT_PATH . '/includes/cls_image.php');//会员头像 by yuanzb
$image = new cls_image($_CFG['bgcolor']);//会员头像 by yuanzb
$allow_suffix = array('gif', 'jpg', 'png', 'jpeg', 'bmp');//会员头像 by yuanzb
...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','act_edit_password2','get_password','reset_pwd','send_pwd_email','password',
    ...
    'check_answer','validate_mobile','get_mobile_code','is_mobile','get_RetrievePwd');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list','default_address',
'message_list', 'tag_list', 'get_password', 'reset_password', 'reset_pwd','booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy',
    ...
    'qpassword_name', 'get_passwd_question', 'check_answer','my_profit','my_score','my_invite');
...
if ($action == 'default')
{
    ...
    $sql = 'SELECT goods_id,goods_name,goods_thumb,market_price,shop_price,click_count FROM '. $ecs->table('goods') .' WHERE goods_number > 0 AND is_on_sale = 1 AND is_delete = 0 AND supplier_id = 0 ORDER BY click_count DESC LIMIT 0,8';
    $click_count = $db->getAll($sql);
    $sql = 'SELECT status FROM ' . $ecs->table('supplier')." WHERE user_id = ".$_SESSION['user_id'];
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $username = isset($_POST['mobile_phone']) ? $_POST['mobile_phone'] : '';
        ...
        if($mobile_code != $_SESSION['mobile_code']){
            show_message($_LANG['username_no_captcha']);
        }
...
elseif($action=='get_mobile_code'){
    $mobile=$_POST['mobile'];
    $sms = new Usms();
    $rlt = $sms->sendSmsReg($mobile,$_SESSION['send_code']);
...
/*找回密码手机验证码*/
elseif($action=='get_RetrievePwd'){
    $mobile=$_POST['mobile'];
    $sms = new Usms();
    $rlt = $sms->RetrievePwd($mobile,$_SESSION['send_code']);

    echo json_encode($rlt);
}

/*验证手机号是否唯一*/
elseif($action=='validate_mobile'){
    $mobile=$_POST['mobile'];
    $res=check_mobile($mobile);
    if($res){
        echo json_encode('手机号已存在，请重新输入');
    }else{
        echo '0';
    }
...
elseif ($action == 'act_login')
{
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        if(!empty($res)){
            foreach ($res as $re) {
                $sql="SELECT * FROM ".$GLOBALS['ecs']->table('cart')." WHERE user_id=".$_SESSION['user_id']." AND goods_id=".$re['goods_id'];
                $cart=$GLOBALS['db']->getRow($sql);
                if(!empty($cart)){
                    $sql="UPDATE".$GLOBALS['ecs']->table('cart').
                        "SET goods_number=goods_number+".$re['goods_number'].".
                             WHERE rec_id=".$cart['rec_id'];
                    $GLOBALS['db']->query($sql);

                    $sql="DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE rec_id=".$re['rec_id'];
                    $GLOBALS['db']->query($sql);
        ...
        $_SESSION['supplier_id']=$res['supplier_id'];
...
elseif ($action == 'act_edit_profile')
{
    ...
    $avatar = isset($_POST['avatar']) ? $_POST['avatar'] : '';//会员头像 by yuanzb
    ...
    if (!empty($_FILES['avatar']['name']))
    {
        ...
        $a=explode('.', $_FILES['avatar']['name']);
        $img_name = $user_id . '.' . end($a);
...
elseif($action == 'reset_pwd'){
    ...
    if($captcha!=$_SESSION['mobile_code']){
        show_message($_LANG['username_no_captcha']);
    }
    $sql = "SELECT user_id, user_name, email,sex,birthday,reg_time,password,mobile_phone  FROM " . $ecs->table('users') .
               " WHERE mobile_phone =".$mobile_phone;
    $user_info = $GLOBALS['db']->getRow($sql);

    // $user_info = $user->get_user_info($mobile_phone);
     

    // if ($user_info && $user_info['captcha'] == $captcha)
    if ($user_info)
    {
        //生成code
         //$code = md5($user_info[0] . $user_info[1]);
    ...
    {
        //用户名与邮件地址不匹配
        show_message($_LANG['username_no_captcha'], $_LANG['back_page_up'], '', 'info');
    }
...
elseif ($action == 'act_edit_password2')//增加 by zhuyu
{
    include_once(ROOT_PATH . 'includes/lib_passport.php');
    $new_password = isset($_POST['new_password']) ? trim($_POST['new_password']) : '';
    $user_id      = isset($_POST['uid'])  ? intval($_POST['uid']) : $user_id;
    ...
    if (($user_info) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'])))
    {
        
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'password'=>$new_password), empty($code) ? 0 : 1))
        {
...
elseif ($action == 'order_list')
{
    ...
    @$status = $_GET['status'];
    /*增加代码 分页数据计算 by yuanzb  */
    if ($status == 10) {
        $order_status = 5;
        $pay_status = 2;
        $shipping_status = 2;
        $where = "order_status in (1,5) AND shipping_status=" . $shipping_status . " AND pay_status=" . $pay_status . " AND ";
    } elseif ($status == 1) {
        $order_status = 2;
        $pay_status = 0;
        $where = "order_status<" . $order_status . " AND pay_status=" . $pay_status . " AND ";

    } elseif ($status == 2) {
        $pay_status = 2;
        $where = "order_status<=1 AND shipping_status in (0,3) AND pay_status=" . $pay_status . " AND ";
    } elseif ($status == 3) {
        $pay_status = 2;
        $where = "order_status in (1,5) AND shipping_status in (1,4) AND pay_status=" . $pay_status . " AND ";
    }else{
        $where = "order_status in (0,1,5) AND ";
    }
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE ".$where); //修改 yuanzb
    if(!empty($status)){
        $pager  = get_pager('user.php', array('act' => $action,'status'=>$status), $record_count, $page);//修改 yuanzb
    }else{
        $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);//修改 yuanzb
    }

    $orders = get_user_orders($user_id, $pager['size'], $pager['start'],$status); //修改 yuanzb

    $merge  = get_user_merge($user_id);
    //获取订单中的每个商品信息 by yuanzb
    foreach ($orders as $key => $order) {
        $sql = "SELECT og.*,g.goods_thumb,g.goods_name,g.goods_sn,g.goods_id FROM " .
                $GLOBALS['ecs']->table('order_goods'). "AS og LEFT JOIN ".
                $GLOBALS['ecs']->table('goods')." AS g ON og.goods_id = g.goods_id  WHERE og.order_id = " . $order['order_id'];
//        $orders[$key]['goods_info'] = $GLOBALS['db']->getAll($sql);
        $goods = $GLOBALS['db']->getAll($sql);

        if(!empty($goods)){
            $orders[$key]['goods_info'] = $goods;
        }
    }
    /*增加代码 by yuanzb*/
...
/* 查看我的收益 增加 by zhuyu 2016-3-21 09:51:14 */
elseif ($action == 'my_profit'){
    $smarty->display('user_transaction.dwt');
}

/* 查看我的积分 增加 by zhuyu 2016-3-21 09:51:14 */
elseif ($action == 'my_score'){
    // 增加代码 by yangxiaohui
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('points'). " WHERE user_id = '$user_id'");
    $pager = get_pager('user.php', array('act' => $action), $record_count, $page, 10);
    $points = points_pages($user_id, $pager['size'], $pager['start']);
    $point= $db->getAll("SELECT * FROM " .$ecs->table('points'). " WHERE user_id = '$user_id' ORDER BY date ASC");

    $all_sql="SELECT pay_points FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id=".$user_id;
    $all=$GLOBALS['db']->getOne($all_sql);
    /*foreach($point as $k=>$v)
    {
        $pay_points+=$v['pay_points'];
        $rank_points+=$v['rank_points'];
        $all=$pay_points+$rank_points;
    }*/
    $smarty->assign('pager',$pager);
...
/* 查看我的邀请 增加 by zhuyu 2016-3-21 09:51:14 */
elseif ($action == 'my_invite'){
    $smarty->display('user_transaction.dwt');
}
...
elseif ($action == 'order_detail')
{
    ...
    $consignee['country']  = isset($order['country'])  ? intval($order['country'])  : 0;
    $consignee['province'] = isset($order['province']) ? intval($order['province']) : 0;
    $consignee['city']     = isset($order['city'])     ? intval($order['city'])     : 0;  
    @$province[$region_id] = get_regions(1, $consignee['country']);
    $city[$region_id]     = get_regions(2, $consignee['province']);
    $district[$region_id] = get_regions(3, $consignee['city']);
    ...
    {
        $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('goods') . " WHERE goods_id =". $value['goods_id'];
        $res[] = $GLOBALS['db']->getRow($sql);
        $goods_list[$key]['goods_tax'] = $res[$key]['goods_tax'];
        $goods_list[$key]['tax_value'] =$res[$key]['goods_tax'] / 100 * $value['goods_price'] * $value['goods_number'];
        $goods_list[$key]['tax_fee']=price_format($goods_list[$key]['tax_value']);
        $goods_list[$key]['market_price'] = price_format($value['market_price'], false);
        $goods_list[$key]['goods_price']  = price_format($value['goods_price'], false);
        // $goods_list[$key]['subtotal']     = price_format($value['subtotal'], false);
        $goods_list[$key]['subtotal']     = price_format($value['goods_price'] +  $goods_list[$key]['tax_value'], false);
    }
     /* 设置能否修改使用余额数 */
    ...
    $smarty->assign('province',    $province[$region_id]);
    $smarty->assign('city',        $city[$region_id]);
    $smarty->assign('district',    $district[$region_id]);
...
elseif ($action == 'comment_list')
{
   ...
   if(@$status=="1")//发表评论
   {
        ...
        $sql="SELECT order_id,email,user_id FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id=".$order_id;
        ...
        foreach($order_id as $k2=>$v2){
            ...
            $user_name=$db->getAll($sql);

            foreach($user_name as $k4=>$v4){
                $order_data['user_name']=$v4['user_name'];
            }
        }      
        $sql="SELECT goods_sn FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id=".$goods_id;
        $goods_sn=$db->getAll($sql);

        foreach($goods_sn as $k4=>$v4){
             $order_data['goods_sn']=$v4['goods_sn'];
        }
        ...
        'user_id'       => $user_id,
        'goods_sn'      => $order_data['goods_sn'],
        'email'         => $order_data['email'],
        'user_name'     => $order_data['user_name'],
        'add_time'      => time(),
        'ip_address'    => $ip,
        'id_value'      => $goods_id,
        'comment_rank'  => $_POST['mark'],
        'order_id'      => $_GET['order_id']
    );

        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('comment'), $comment_info,'INSERT');
         $sql="UPDATE ".$GLOBALS['ecs']->table('order_goods')." SET is_comment=1 WHERE order_id=".$order_data['order_id']." AND goods_sn=".$order_data['goods_sn'];
       $GLOBALS['db']->query($sql);
        ecs_header("Location: user.php?act=comment_list&status=2\n");
        exit;
   }
   if(@$status=="2")//已评论
   {
    ...
    $pager = get_pager('user.php', array('act' => $action.'&status=2'), $record_count, $page, 5);

    $comment=get_comment_list($user_id, $pager['size'], $pager['start']);
    foreach($comment as $key2=>$val2){
        $sql="SELECT order_id,add_time,order_sn FROM ".$GLOBALS['ecs']->table('order_info')." WHERE user_id=".$val2['user_id']." AND order_id=".$val2['order_id'];
        $order_info=$GLOBALS['db']->getAll($sql);

        foreach($order_info as $key3=>$val3)
        {
            $sql =  " SELECT goods_id,goods_sn,goods_thumb,goods_name FROM ". $GLOBALS['ecs']->table('goods'). " WHERE goods_id = " .$val2['id_value'];
            $goods= $GLOBALS['db']->getAll($sql);
            foreach($goods as $k=>$v){
                $v['order_id']=$val3['order_id'];
                $v['addtime']=date("Y-m-d H:i:s",$val3['add_time']);
                $v['order_sn']=$val3['order_sn'];
                $v['content']=$val2['content'];
                $comment_list[]=$v;
   ...
   elseif(@$status=="3")//未评论
   {
        $comment=get_comment_list($user_id, 10, 0);
        if(!empty($comment)) {
            foreach ($comment as $k7 => $v7) {
                $good_id[] = $v7["id_value"];
                $order_ids[] = $v7['order_id'];
            }
            $goods_id = implode(" AND goods_id!=", $good_id);
            $order_id = implode(" AND order_id!=", $order_ids);

            if (!empty($order_id)) {

                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og LEFT JOIN ". $GLOBALS['ecs']->table('order_info') ." AS oi ON og.order_id = oi.order_id WHERE oi.user_id ='$user_id' AND og.shipping_status=2 AND og.is_comment = 0";
                $record_count = $db->getOne($sql);

                $pager = get_pager('user.php', array('act' => $action . '&status=3'), $record_count, $page, 5);
                // print_r($pager);die;
                if ($pager['start'] == 0) {
                    $sql = "SELECT og.goods_id,og.order_id FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og LEFT JOIN ". $GLOBALS['ecs']->table('order_info') ." AS oi ON og.order_id = oi.order_id  WHERE oi.user_id ='$user_id' AND og.shipping_status=2 AND og.is_comment = 0 ORDER BY oi.add_time DESC LIMIT "  . $pager['start'] . ', '. $pager['size'];
                } else {
                    $sql = "SELECT og.goods_id,og.order_id FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og LEFT JOIN ". $GLOBALS['ecs']->table('order_info') ." AS oi ON og.order_id = oi.order_id  WHERE oi.user_id ='$user_id' AND og.shipping_status=2 AND og.is_comment = 0 ORDER BY oi.add_time DESC LIMIT " . $pager['start'] . ', ' . $pager['size'];
                }
            } else {
                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_goods') . " WHERE user_id ='$user_id' AND shipping_status=2";
                $record_count = $db->getOne($sql);

                $pager = get_pager('user.php', array('act' => $action . '&status=3'), $record_count, $page, 5);
                if ($pager['start'] == 0) {
                    $sql = "SELECT goods_id,order_id FROM " . $GLOBALS['ecs']->table('order_goods') . " WHERE user_id ='$user_id' AND shipping_status=2  ORDER BY oi.add_time DESC LIMIT " . $pager['start'] . ', ' . $pager['size'];
                } else {
                    $sql = "SELECT goods_id,order_id FROM " . $GLOBALS['ecs']->table('order_goods') . " WHERE user_id ='$user_id' AND shipping_status=2  ORDER BY oi.add_time DESC LIMIT " . $pager['start'] . ', ' . $pager['size'];
                }
            }
            ...
            foreach ($res as $key1 => $re) {
                $sql = " SELECT goods_id,goods_sn,goods_thumb,goods_name FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = " . $re['goods_id'];
                $goods = $GLOBALS['db']->getAll($sql);
                foreach ($goods as $k => $v) {
                    $v['order_id'] = $re['order_id'];


                    $order_sn = $db->getAll("SELECT order_sn,add_time FROM " . $ecs->table('order_info') . " WHERE order_id=" . $re['order_id']);
                    foreach ($order_sn as $key2 => $val2) {
                        $v['order_sn'] = $val2['order_sn'];
                        $v['addtime'] = date("Y-m-d H:i:s", $val2['add_time']);
                    }
   ...
   else
   {
    ...
    // /* 获取用户留言的数量 */
        if($pager['start']==0){
            $sql = "SELECT goods_id,order_id FROM ". $GLOBALS['ecs']->table('order_goods'). " WHERE user_id ='$user_id' AND shipping_status=2 LIMIT ". $pager['size']." ORDER BY oi.add_time DESC";
        }else{
             $sql = "SELECT goods_id,order_id FROM ". $GLOBALS['ecs']->table('order_goods'). " WHERE user_id ='$user_id' AND shipping_status=2 LIMIT " . $pager['start'] . ', '. $pager['size']." ORDER BY oi.add_time DESC";
        }
        ...
        foreach($res as $key1 => $re){
            ...
            $goods= $GLOBALS['db']->getAll($sql);
                ...
                foreach($goods as $k=>$v){
                     ...
                     $content=$db->getAll("SELECT content FROM".$ecs->table('comment')." WHERE order_id=".$re['order_id']." AND id_value=".$re['goods_id']);
