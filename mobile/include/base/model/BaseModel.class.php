...
class BaseModel extends Model {
    ...
    function debug_log($info,$file){ //TODO：调试代码
        $type=gettype($info);
        $info=$type.json_encode($info);
        $sql="INSERT INTO slb_error_log (info,file,time) VALUES('".$info ."','".$file."','".time()."')";
        $this->query($sql);
    }
    /**
     * 查询全部分类列表
    ...
    public function load_config() {
        ...
        if ($data === false) {
            ...
            $arr['no_picture'] = !empty($arr['no_picture']) ? str_replace('./', '/', $arr['no_picture']) : __ROOT__.'/data/common/images/no_picture1.png'; // 修改默认商品图片的路径
