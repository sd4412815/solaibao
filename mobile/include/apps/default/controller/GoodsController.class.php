...
class GoodsController extends CommonController {
    ...
    public function index() {
        ...
        $sales_count =model('Goods')->row($sql);
        // 当前系统时间
        ...
        $this->assign('goods_tax',$goods['goods_tax']);
...
//          }
    ...
    public function prices() {
        ...
        $nowamount = (isset($_REQUEST ['nowamount'])) ? intval($_REQUEST ['nowamount']) : '';
        $nowprice = (isset($_REQUEST ['nowprice'])) ? intval($_REQUEST ['nowprice']) : '';
        // 如果商品id错误
        if ($this->goods_id == 0) {
            $res ['err_msg'] = L('err_change_attr');
            $res ['err_no'] = 1;
