...
class UserController extends CommonController {
    ...
    public function index() {
        ...
        if($user_info = model('Users')->get_profile($this->user_id)){
            $this->assign('user_info',$user_info);
        }

        $this->assign('base_path',C('SHOP_URL'));
        // 收藏
    ...
    public function profile() {
        ...
        if (IS_POST) {
            $email = I('post.email');
            $birthday[]=I('post.birthdayYear');
            $birthday[]=I('post.birthdayMonth');
            $birthday[]=I('post.birthdayDay');
    ...
    public function not_pay_order_list() {
        ...
        $count = $this->model->table('order_info')->where('user_id = ' . $this->user_id.' AND pay_status < 2')->count();
        $filter['page'] = '{page}';
        $offset = $this->pageLimit(url('not_pay_order_list', $filter), $size);
        $offset_page = explode(',', $offset);
        $length = 5;
        $limit = $this->pageLimit(url('not_pay_order_list', array('sort' => $this->sort, 'order' => $this->order)), $this->length);
        $orders = model('Users')->get_user_orders($this->user_id, $pay, $offset_page[1], 4,$limit);
        $goods_search = model('Goodslist')->goodslist_get_search();
        //获取订单中的每个商品信息 by yuanzb
        foreach ($orders as $key => $order) {
            $sql = "SELECT og.*,g.goods_thumb,g.goods_name,g.goods_sn,g.goods_id FROM " .
                M()->pre. "order_goods AS og LEFT JOIN ".
                M()->pre."goods AS g ON og.goods_id = g.goods_id  WHERE og.order_id = " . $order['order_id'];
    ...
    public function order_list() {
        ...
        $offset_page = explode(',', $offset);
        $length = 5;
        $limit = $this->pageLimit(url('order_list', array('sort' => $this->sort, 'order' => $this->order)), $this->length);
        $orders = model('Users')->get_user_orders($this->user_id, $pay, $offset_page[1], $offset_page[0],$limit);
        $goods_search = model('Goodslist')->goodslist_get_search();
        $this->assign('goods_search', $goods_search);
        ...
        // $assign = $this->get_user_orders($user_id);
        $this->assign('path',C('SHOP_URL'));
        $this->assign('pay', $pay);
    ...
    public function order_detail() {
       ...
       if ($order['order_status'] == "等待收货") {

            /* 对配送状态的处理 */
            if ($order['shipping_status'] == 1) {                                    
            @$order['handler'] = "<a id=\"user_order_hide\" class=\"btn btn-info ect-colorf\" href=\"" . url('user/affirm_received', array(
                            'order_id' => $order['order_id']
                        )) . "\" onclick=\"if (!confirm('" . L('confirm_received') . "')) return false;\">" . L('received') . "</a>";
            } 

        }else if ($order['order_status'] == "等待支付") {
            $order['handler'] = "<a id=\"user_order_hide\" style=\"background-color:#e95466;\" class=\"btn btn-info ect-colorf\" href=\"" . url('user/cancel_order', array(
                        'order_id' => $order['order_id']
                    )) . "\" onclick=\"if (!confirm('" . L('confirm_cancel') . "')) return false;\">" . L('cancel') . "</a>";

        } elseif ($order['shipping_status'] == SS_RECEIVED) {

//                @$order['handler'] = '<a id="user_order_hide" class="btn btn-info ect-colorf" type="button" href=".url(\'comment/comment\',array(\'oid\'=>$order[\'order_id\']])).">' . L('go_comment') . '</a>';
           @$order['handler'] ='';
        } else {

                if ($order['pay_status'] == PS_UNPAYED) {

                    @$order['handler'] = "<a id=\"bianse\" class=\"btn btn-infoect-colorf\" href=\"" . url('user/cancel_order', array(
                                'order_id' => $order['order_id']
                            )) . "\">" . L('pay_money') . "</a>";
                } else {
                    @$order['handler'] = "<a id=\"user_order_hide\" class=\"btn btn-info ect-colorf\" href=\"javascript:void(0);\">" . L('view_order') . "</a>";
        ...
        foreach ($goods_list as $key => $value) {
            ...
            $goods_list[$key]['goods_tax']=$value['goods_tax'];
        }
        ...
        $goods_search = model('Goodslist')->goodslist_get_search();
        $this->assign('goods_search', $goods_search);
        $order['province_name'] = model('Users')->get_order_addres($order['province']);   //province   city   district
        $order['city_name'] = model('Users')->get_order_addres($order['city']);   //province   city   district
        $order['district_name'] = model('Users')->get_order_addres($order['district']);   //province   city   district
    ...
    public function affirm_received() {
        ...
        $url="index.php?m=default&c=user&a=order_detail&order_id=".$order_id;
    ...
    public function edit_payment() {
        ...
        if ($order['pay_status'] != PS_UNPAYED || $order['shipping_status'] != SS_UNSHIPPED || $order['pay_id'] == $pay_id) {
            $url = url('order_detail', array(
                'order_id' => $order_id
            ));
            ecs_header("Location: $url\n");
        ...
        $where['order_id'] = $order_id;
        $res=$this->model->table('order_info')
                ->data($data)
                ->where($where)
                ->update();
    ...
    public function edit_address() {
        ...
        if (IS_POST) {
            $address = array(
                ...
                'consignee' => I('post.consignee'),
                'mobile' => I('post.mobile'),
                 'id_card'=>I('post.id_card')//by yangxioahui
            );
    ...
    //设置默认地址
    public function change_address_id(){
        $address_id=$_POST['default_address_id'];
        ...
        if($from=="cart"){
             ecs_header("Location:index.php?m=default&c=flow&a=checkout");
           
        }else{
           
             ecs_header("Location:index.php?m=default&c=user&a=index");
        }
        exit();
    ...
    public function earnings(){
        ...
        $goods_search = model('Goodslist')->goodslist_get_search();
        $this->assign('goods_search', $goods_search);
        $this->assign('word',敬请期待。。。);
        $this->display('earnings.dwt');
    } 
    //我的邀请
    public function invite(){
        ...
        $goods_search = model('Goodslist')->goodslist_get_search();
        $this->assign('goods_search', $goods_search);
        $this->display('invite.dwt');
    }
    //我的积分
    public function integral(){
        $pay = 1;
        $size = I(C('page_size'), 5);
        $count = $this->model->table('points')->where('user_id = ' . $this->user_id)->count();
        $filter['page'] = '{page}';
        $offset = $this->pageLimit(url('integral', $filter), $size);
        $offset_page = explode(',', $offset);
        $length = 5;
        $limit = $this->pageLimit(url('integral', array('sort' => $this->sort, 'order' => $this->order)), $this->length);

        $sql="SELECT * FROM " .$this->model->pre. "points WHERE user_id = ".$_SESSION['user_id']." ORDER BY date DESC LIMIT ".$limit;
        $points=$this->model->query($sql);

        $sql="SELECT pay_points FROM ".$this->model->pre."users WHERE user_id=".$_SESSION['user_id'];
        ...
        $this->assign('all',$all["pay_points"]);
        $this->display('integral.dwt');
    }
    /**
     * 缺货登记列表
     */
   ...
   public function login() {
    ...
    public function check_mobile()
    {
        ...
        $res=model('Users')->check_mobile($mobile);
    ...
    public function register() {
        ...
        if (IS_POST) {
            ...
            $repass = isset($_POST['repass']) ? in($_POST['repass']) : '';
            ...
            /*判断密码和确认密码是否一致*/
            if($password!=$repass){
                show_message('两次密码不一致', L('register_back'), url('register'), 'error');
            }
            /*判断密码是否大于6位*/
            if(strlen($password)<6){
                show_message('请输入大于6位的密码', L('register_back'), url('register'), 'error');
            }
            /*判断手机验证码是否正确*/
            ...
            {
                $extend_field_index = 'extend_field' . $val['id'];
                $_POST[$extend_field_index]=$mobile;
            ...
            if (model('Users')->register($username, $password, $other) !== false) {
    ...
    public function get_password_phone() {
        ...
        if (IS_POST) {
            ...
            $is_mobile=model('Users')->check_mobile($mobile);
            if($is_mobile==0){
                show_message(L('msg_mobile'), L('back_page_up'), url('get_password_phone'), 'error');
            }
...
// YWT_增加代码
    public function send_pwd_email() {
        ...
        //     }
            ...
            // }
        // }
        // $user_id = !empty($_POST['user_name']) ? in($_POST['user_name']) : '';
        $mobile = $_POST['mobile'];
        $sql = "SELECT user_id,mobile_phone,user_name FROM " . $this->model->pre ."users". " WHERE mobile_phone = " .$mobile ;
        if(!empty($mobile)){
            $res = $this->model->getRow($sql);
        }else{
            show_message(L('username_no_mobile') , L('back_retry_answer'), url('get_password_email'), 'info');
        }
        $mobile_phone = $res['mobile_phone'];
        if($mobile_phone==$mobile){
            $this->assign('user_id',$res['user_id']);
            $this->assign('user_name',$res['user_name']);
            $this->display('user_edit_psw.dwt');
        }else{
             show_message(L('username_no_mobile') , L('back_retry_answer'), url('get_password_email'), 'info');
        }
      
    }
    public function send_pwd_edit() {
            $goods_search = model('Goodslist')->goodslist_get_search();
            $this->assign('goods_search', $goods_search);
           // var_dump($_SESSION['user_id']);
            ...
            $this->display('user_edit_pwd.dwt');
    }
    public function send_updatePsw(){
        ...
        //         show_message(L('passport_js.password_length'));
        //     }
        // if($new_password == $comfirm_password){
            
        //    if (self::$user->edit_user(
        //                     array( 'username' => $_SESSION['user_name'],'password' => $new_password),  empty($mobile) ? 0 : 1)) {
        //             $data['ec_salt'] = 0;
        ...
        //                     ->update();
        //             // self::$user->logout();
        //             show_message(L('edit_password_success'), L('back_home_personal'), url('user/index'), 'info');
        //         } else {
        //             show_message(L('edit_password_failure'), L('back_page_up'), '', 'info');
        //         }
           
        //     show_message(L('oldpassword_success') ,L('back_relogin_lnk'), url('login'), 'info');
        // }else{
        //     show_message(L('reedit_password') , L('back_retry_answer'), url('send_pwd_edit'), 'info');
        // }
         if (IS_POST) {
            ...
            $sql="SELECT password FROM ". $this->model->pre."users where user_id = $user_id";
            $password = $this->model->query($sql);
            $sql="SELECT ec_salt FROM ". $this->model->pre."users where user_id = $user_id";
            $ec_salt = $this->model->query($sql);
            //$old_password=md5(md5($old_password).$ec_salt);
            foreach($ec_salt as $k1=>$v1){
                $id=$v1['ec_salt'];
            }
          
        
             $old_password=md5(md5($old_password).$id);

               
            foreach($password as $k=>$v){
               
                if($old_password!==$v['password']){
                    show_message('输入的原密码不正确');
                }

            }
            if (strlen($new_password) < 6) {
              
                show_message('密码长度不少于6位');
            }
            ...
            if($comfirm_password!==$new_password){
                ...
                $new_password=md5(md5($new_password).$id);
                $sql="UPDATE ".$this->model->pre."users SET password='$new_password' WHERE user_id=$user_id";
                $npwd = $this->model->query($sql);
                //p($npwd);
                if($npwd){
                 // if (self::$user->edit_user(
                 //    array( 'username' => $_SESSION['user_name'],'password' => $new_password),  empty($mobile) ? 0 : 1)) {
    ...
    public function send_updatePwd(){
        ...
        if($new_password == $comfirm_password){
           if (self::$user->edit_user2(
                            array( 
                                'username' => ((empty($code) && empty($mobile) && empty($question)) ? $username : $user_info['user_name']),
            ...
            show_message(L('reedit_password') , L('back_retry_answer'), url('get_password_phone'), 'info');
        }
    }
    public function send_pwd_phone() {
        $captcha = intval(C('captcha'));
        if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2))) {
            if (empty($_POST['captcha'])) {
                show_message(L('invalid_captcha'), L('back_page_up'), url('get_password_email'), 'error');
...
//     $res = $db->getOne($sql);
//     v($res);
//     $user_info = $user->get_user_info($user_mobile);
//     if ($user_info && $user_info['mobile'] == $mobile)
//     {
//         //生成code
//          //$code = md5($user_info[0] . $user_info[1]);
//         $code = md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']);
//         //发送的短信
//         if (send_pwd_phone($user_info['mobile'],$code))
//         {
//             show_message($_LANG['send_success_mobile'] . $mobile, $_LANG['back_home_lnk'], './', 'info');
//         }
//         else
//         {
//             //短信发送出错
//             show_message($_LANG['mobile_send_password'], $_LANG['back_page_up'], './', 'info');
//         }
//     }
//     else
//     {
...
// }
    ...
    public function edit_password() {
        ...
        if (IS_POST) {
            ...
            // 登录修改密码
            if ( $_SESSION['user_id'] > 0 && $_SESSION['user_id'] == $user_id && self::$user->check_user($_SESSION['user_name'], $old_password)) {
