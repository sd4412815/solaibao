...
class FlowController extends CommonController {
        ...
        if (isset($_POST ['rec_id']) && isset($_POST ['goods_number'])) {
            ...
            if ($val > 0) {
                ...
                foreach ($offers_accessories_res as $offers_accessories_row) {
                    if ($row_num > $val) {
                        $sql = "DELETE FROM" . $this->model->pre . "cart WHERE user_id='" . $_SESSION['user_id'] . "'' " . " AND rec_id ='" . $offers_accessories_row ['rec_id'] . "' LIMIT 1";
                ...
                if ($goods ['extension_code'] == 'package_buy') {
                    ...
                    // 更新购物车中的商品数量
                    $sql = "UPDATE " . $this->model->pre . "cart SET goods_number= '$val', goods_price = '$goods_price' WHERE rec_id='$key' AND user_id='" . $_SESSION['user_id'] . "'";
                }
            }  // 订货数量等于0
            else {
            ...
            $this->model->query($sql);
            /* 删除所有赠品 */
            $sql = "DELETE FROM " . $this->model->pre . "cart WHERE user_id='" . $_SESSION['user_id'] . "' AND is_gift <> 0";
            $this->model->query($sql);
    ...
    public function checkout() {
        ...
        $address_id = $this->model->table('users')->field('address_id')->where("user_id = '" . $_SESSION['user_id'] . "' ")->getOne();
         ...
         //by yangxiaohui
        $id_card= $this->model->table('user_address')->field('id_card')->where("address_id = '" . $address_id. "' ")->getOne();
        $consignee['id_card']=$id_card;
       ...
       $this->assign('goods_list', $cart_goods);
        ...
        /* 取得可以得到的积分和红包 */
        $total_integral=model('Order')->cart_amount(false, $flow_type) - $total ['bonus'] - $total ['integral_money']-$discount ['discount'];
        ...
        foreach($cart_id as $k=>$v){
            ...
            $condition = "`user_id` = '" .$_SESSION['user_id'] . "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND `rec_id`= $rec_id";
        ...
        $this->assign('shipping_count', $shipping_count);
        ...
        if ((!isset($use_integral) || C('use_integral') == '1') && $_SESSION ['user_id'] > 0 && $user_info ['pay_points'] > 0 && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
            ...
            $this->assign('order_max_integral', model('Flow')->flow_available_points($cart_id)); // 可用积分
    ...
    public function consignee() {
        if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
            ...
            $consignee = array(
                ...
                'id_card'=>empty($_POST['id_card'])?'':I('post.id_card')
            );
    ...
    public function change_address_id(){
        ...
        model('Order')->save_consignee_default($address_id);
        if($from=="cart"){
    ...
    public function select_shipping() {
        ...
        if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
            ...
            /* 取得可以得到的积分和红包 */
             ...
             $total_integral=model('Order')->cart_amount(false, $flow_type) - $total ['bonus'] - $total ['integral_money'];
            $total_integral=floor($total_integral);
            $this->assign('total_integral',  $total_integral);
    ...
    public function done() {
        ...
        if ($user_id > 0) {
            ...
            // 查询用户有多少积分
            $cart_id = explode(".",$_COOKIE['spid']);
            $flow_points = model('Flow')->flow_available_points($cart_id); // 该订单允许使用的积分
 ...
 //var_dump($total); 
        /* 订单中的总额 */
       // $total = model('Users')->order_fee($order, $cart_goodses, $consignee);
        $order ['bonus'] = $total ['bonus'];
        $order ['goods_amount'] = $total ['goods_price'];
        $order ['discount'] = $total ['discount'];
        $order ['surplus'] = $total ['surplus'];
        $order ['tax_fee'] = $total ['tax_fee'];
        ...
        //$order ['integral_money'] = $order['integral'];
       ...
       // var_dump($order);
        ...
        foreach($cart_id as $k=>$v){
            $rec_id=$v;  
        /* 插入订单商品 */
        $sql = "INSERT INTO " . $this->model->pre . "order_goods( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) " . " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id" . " FROM " . $this->model->pre . "cart WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$flow_type' AND rec_id='$rec_id'";
        $this->model->query($sql);
        }
        /* 修改拍卖活动状态 */
      ...
      // 销量
        ...
        //yang
         foreach($cart_id as $k=>$v){
            $rec_id=$v;
            model('Order')->clear_cart($flow_type,$rec_id);
        }
        //yang
