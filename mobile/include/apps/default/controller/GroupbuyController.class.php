...
class GroupbuyController extends CommonController {
    ...
    public function info() {
        ...
        $nowprice = model('Groupbuy')->group_buy_nowprice($group_buy_id);
        $group_buy['gmt_end_date'] = $group_buy['end_date'];
        // p($group_buy); 
        // v(json_decode($group_buy['ext_info']));
        $this->assign('group_buy', $group_buy);
        $this->assign('group_buy_nowprice', $nowprice);
        /* 取得团购商品信息 */
    ...
    public function buy() {
        ...
        /* 更新：清空购物车中所有团购商品 */
        model('Order')->clear_other_cart(CART_GROUP_BUY_GOODS);
        ...
        $sql="SELECT rec_id FROM ".$this->model->pre."cart WHERE user_id=".$_SESSION['user_id']." AND goods_id=".$group_buy['goods_id'];
        $arr=$this->model->query($sql);
        foreach($arr as $k=>$v){
           
            $rec_id=$v['rec_id'];
        }
         setcookie("spid",$rec_id);
