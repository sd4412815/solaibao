...
class UsersModel extends BaseModel {
    ...
    {
        $sql="SELECT mobile_phone FROM " .$this->pre."users WHERE mobile_phone=".$mobile;
        $mobile=$this->row($sql);
    ...
    function register($username, $password, $other = array()) {
        ...
        if (!ECTouch::user()->add_user($username, $password)) {
    ...
    function edit_profile($profile) {
        ...
        $cfg = array();
        $user_name=$this->clean($profile['user_name']);
        $sql="UPDATE " . $this->pre ."users SET user_name='$user_name' WHERE user_id=".$profile['user_id']; 
        $a=$this->query($sql);
        //var_dump($a);
        $sql = "SELECT user_name FROM " . $this->pre . "users WHERE user_id='" . $profile['user_id'] . "'";
        $res = $this->row($sql);

        //$res= $this->clean($profile['user_name']);
    ...
    function get_profile($user_id) {
        ...
        $info = array();
        $infos = array();
        $sql = "SELECT user_name, birthday,avatar, sex, question, answer, rank_points, pay_points,user_money, user_rank," .
        ...
        $_SESSION['user_name']=$infos['user_name'];//yang,改变session
    ...
    function get_user_orders($user_id, $num = 10, $start=0 , $status=null,$limit = null)
    {
    /* 取得订单列表 */
    $arr    = array();
    /*YWT_增加代码   */
    ...
    $sql = "SELECT order_id, order_sn,consignee, order_status, shipping_status, pay_status, add_time,shipping_id, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
           " FROM " . $this->pre .
           "order_info WHERE ".$where." ORDER BY add_time DESC ".$limit;
...
/*YWT_增加代码 */
    ...
    foreach ($res as $key => $row) {
         /*增加代码 by yuanzb  */
        if($row['pay_status'] == 0 && $row['order_status'] < 2 ){
            $row['order_status'] = '等待支付';
            $row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>'
            .'<br />'."<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }elseif ($row['pay_status'] == 2 && ( $row['shipping_status'] == 2) && ($row['order_status'] == 1||$row['order_status'] == 5)) {
            $row['order_status'] = '已完成';
            @$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
        }elseif ($row['pay_status'] == 2 && ($row['shipping_status'] == 1 ||$row['shipping_status'] == 4) && ($row['order_status'] == 1||$row['order_status'] == 5)) {
            $row['order_status'] = '等待收货';
           ...
           // $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>"; //取消
        }elseif($row['pay_status'] == 2 && ($row['shipping_status'] == 0) && $row['order_status'] <= 1){
            $row['order_status'] = '等待发货';
            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>"; //取消
        }elseif($row['pay_status'] == 2 && ( $row['shipping_status'] == 5) && ($row['order_status'] == 2)){
            $row['order_status'] = '配货中';
        }elseif($row['pay_status'] == 2 && ( $row['shipping_status'] == 3) && ($row['order_status'] == 1)){
            $row['order_status'] = '配货中';
        }elseif($row['pay_status'] == 2 && ( $row['shipping_status'] == 5) && ($row['order_status'] == 5)){
            $row['order_status'] = '发货中';
...
//                       'img'            => get_image_path(0, model('Order')->get_order_thumb($row['order_id'])),
                       ...
                       'handler'        => $row['handler'],
                        'is_comment'    => model('Order')->order_goods($row['order_id'])['0']['is_comment']
                        );
        }
    return $arr;
}
    ...
    function affirm_received($order_id, $user_id = 0) {
        /* 查询订单信息，检查状态 */
        $sql = "SELECT user_id, order_id, order_sn , order_status, shipping_status, pay_status FROM " . $this->pre . "order_info WHERE order_id = '$order_id'";
        ...
        /* 修改订单发货状态为“确认收货” */ else {
            ...
            if ($this->query($sql)) {
                ...
                model('ClipsBase')->log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf('订单 '.$order['order_sn'].'赠送的积分', $order['order_sn']));
                $time=date("Y-m-d H:i:s");

            
                $sql = "UPDATE " . $this->pre."order_goods SET shipping_status = '" . SS_RECEIVED . "' WHERE order_id = '$order_id'";
                $this->query($sql);
                ...
                if ($code == 'package_buy') {
                    foreach ($goods_list as $goods) {
                        $sql = 'SELECT g.goods_id FROM ' . $this->pre . 'package_goods AS pg, ' . $this->pre . 'goods AS g ' .
                                "WHERE pg.goods_id = g.goods_id AND g.goods_number != 0 AND pg.package_id = '" . $goods['goods_id'] . "' AND extension_code = 'virtual_card'";
...
// p($order);
        ...
        /*增加代码 by yuanzb  */
        if($order['pay_status'] == 0 && $order['order_status'] < 2 ){
            @$order['order_status'] = '等待支付';
            // @$order['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$order['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>'
            // .'<br />'."<a href=\"user.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }elseif ($order['pay_status'] == 2 && ( $order['shipping_status'] == 2) && ($order['order_status'] == 1||$order['order_status'] == 5)) {
            $order['order_status'] = '已完成';
            @$order['handler'] = '<span style="color:red">收货确认</span>';
        }elseif ($order['pay_status'] == 2 && ($order['shipping_status'] == 1 ||$order['shipping_status'] == 4) && ($order['order_status'] == 1||$order['order_status'] == 5)) {
            $order['order_status'] = '等待收货';
            @$order['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
//            $order['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>"; //取消
        }elseif($order['pay_status'] == 2 && ($order['shipping_status'] == 0 ) && $order['order_status'] <= 1){
            $order['order_status'] = '等待发货';
            $order['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".L('confirm_cancel')."')) return false;\">".L('cancel')."</a>"; //取消
        }elseif($order['pay_status'] == 2 && ( $order['shipping_status'] == 5) && ($order['order_status'] == 2)){
            $order['order_status'] = '配货中';
        }elseif($order['pay_status'] == 2 && ( $order['shipping_status'] == 3) && ($order['order_status'] == 1)){
            $order['order_status'] = '配货中';
        }elseif($order['pay_status'] == 2 && ( $order['shipping_status'] == 5) && ($order['order_status'] == 5)){
            $order['order_status'] = '发货中';
        }elseif($order['order_status'] == 2){
            $order['order_status'] = '已取消';
        }elseif($order['order_status'] == 4){
            $order['order_status'] = '退货';
        }
...
/*增加代码 by yuanzb  */
    ...
    function return_to_cart($order_id) {
        ...
        foreach ($res as $row) {
            ...
            $sql = "SELECT goods_sn, goods_name, goods_number, market_price, " .
                    ...
                    "FROM " . $this->pre .
                    "goods WHERE goods_id = '$row[goods_id]' AND goods_number != 0 " .
     ...
     * 
     */
    function save_consignee_default($address) {
        /* 保存为用户的默认收货地址 */
        $sql = "UPDATE " . $this->pre .
    ...
    function order_fee($order, $cart_goods, $consignee) {
        ...
        $total = array('real_goods_count' => 0,
            ...
            'tax_du_formated' => 0);
        $weight = 0;
         /* 海关税额 start by yangxiaohui*/
        foreach ($cart_goods as $key=> $good) {              
            $goods_tax=(int)$good['cart_tax']*(int)$good['goods_price']*(int)$good['goods_number']/100;
            $total_fee+=$goods_tax;
        }
       
        $total=array();
        $total['tax_fee']=$total_fee;
      
        $total['tax_fee_formated']=price_format($total['tax_fee'], false);
    /* 海关税额 end by yangxiaohui*/
        /* 商品总价 */
        ...
        if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0) {
            ...
            if (!empty($shipping_info)) {
                ...
                foreach($cart_id as $k1=>$v1){
                     ...
                     $sql = 'SELECT count(*) as count FROM ' . $this->pre . "cart WHERE  `user_id` = '" . $consignee['user_id'] . "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND `rec_id`=$v1";
                     $res = $this->row($sql);
        ...
        $total['shipping_id']=$order['shipping_id'];
        ...
        if ($order['extension_code'] == 'group_buy' && $group_buy['deposit'] > 0) {
            ...
            //$total['will_get_integral'] = $total['goods_price'];//yang 获得积分
            $total['amount'] = $total['goods_price'] - $total['discount'] + $total['tax_fee'] + $total['pack_fee'] + $total['card_fee'] +
        ...
        /* 取得可以得到的积分和红包 */
        $total_integral=model('Order')->cart_amount(false, $flow_type) - $total ['bonus'] - $total ['integral_money']- $total['discount'];
       
        $total_integral=floor($total_integral);

        // $total['total_integral']= $total_integral;
        $total['will_get_integral']= $total_integral;
        ...
        if ($order['extension_code'] == 'exchange_goods') {
            $sql = 'SELECT eg.exchange_integral ' .
                    'as sum FROM ' . $this->pre . 'cart AS c,' . $this->pre . 'exchange_goods AS eg ' .
                    "WHERE eg.goods_id = ".$order['extension_id']." AND c.session_id= '" . SESS_ID . "' " .
    ...
    function get_order_addres($id){
        $sql="SELECT region_name FROM ".$this->pre."region WHERE region_id=".$id;
        return $this->row($sql)['region_name'];
    }
