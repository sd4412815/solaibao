...
class CategoryModel extends BaseModel {
    ...
    function get_cat_info($cat_id) {
        $sql = 'SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $this->pre . "category WHERE cat_id = '$cat_id'";
        $res = $this->row($sql);
        return $res;
    }
    ...
    function category_get_count($children,$brand, $type, $min, $max, $ext, $keyword) {
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . $this->pre . 'goods AS g ' . ' LEFT JOIN ' . $this->pre . 'touch_goods AS xl ' . ' ON g.goods_id=xl.goods_id ' . ' LEFT JOIN ' . $this->pre . 'member_price AS mp ' . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "WHERE $where $ext AND supplier_id = 0";
        $res = $this->row($sql);
        return $res['count'];
    }
    ...
    function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min = 0, $max = 0, $ext = '') {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.goods_number != 0 AND g.is_delete = 0 ' . $brand_where . $price_where . $ext;
        $num = 0;
