...
class GroupbuyModel extends BaseModel {
    ...
    function group_buy_list($size, $page, $sort, $order) {
        ...
        $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, t.act_banner ,t.sales_count ,t.click_num,  g.market_price , b.act_id AS group_buy_id, " .
                ...
                "WHERE b.act_type = '" . GAT_GROUP_BUY . "' AND g.goods_number != 0 " .
     ...
     * @return type
     */
    function group_buy_nowprice($group_buy_id){
        $sql = "SELECT goods_id  FROM " .$this->pre ."goods_activity WHERE act_id = ".$group_buy_id;       
        $res = $this->row($sql);

        $sql = "SELECT shop_price FROM " . $this->pre ."goods WHERE goods_id =".$res['goods_id'] ;
        $res = $this->row($sql);

        return $res['shop_price'];
    }
}
