...
class GoodsModel extends BaseModel {
    ...
    function get_goods_info($goods_id) {
        ...
        $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
                ...
                "WHERE g.goods_id = '$goods_id' AND g.goods_number != 0 AND g.is_delete = 0 " .
                "GROUP BY g.goods_id";
        $row = $this->row($sql);
    ...
    function get_goods_properties($goods_id) {
        ...
        $sql = "SELECT attr_group " .
                ...
                "WHERE g.goods_id='$goods_id' AND g.goods_number != 0 AND gt.cat_id=g.goods_type";
        $result = $this->row($sql);
        ...
        if (!empty($attr)) {
            foreach ($attr['lnk'] AS $key => $val) {
                ...
                $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                        ...
                        "LEFT JOIN " . $this->pre . "member_price AS mp " .
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                        "WHERE a.attr_id = '$key' AND g.goods_number != 0 AND g.is_on_sale=1 AND a.attr_value = '$val[value]' AND g.goods_id <> '$_REQUEST[id]' " .
    ...
    function get_goods_fittings($goods_list = array()) {
        ...
        $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                ...
                "WHERE gg.parent_id " . db_create_in($goods_list) . " AND g.is_delete = 0 AND g.goods_number != 0 AND g.is_on_sale = 1 " .
    ...
    function get_linked_goods($goods_id) {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                ...
                "LEFT JOIN " . $this->pre . "member_price AS mp " .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
                "WHERE lg.goods_id  " . db_create_in($goodsId) . " AND g.goods_number != 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " .
    ...
    function get_related_goods($goods_id) {
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " . 'g.market_price, g.promote_price, g.promote_start_date, g.promote_end_date ' . 'FROM ' . $this->pre . 'link_goods AS lg ' . 'LEFT JOIN ' . $this->pre . 'goods AS g ON g.goods_id = lg.link_goods_id ' . "LEFT JOIN " . $this->pre . "member_price AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "WHERE lg.goods_id = '$goods_id' AND g.is_on_sale = 1 AND g.goods_number != 0 AND g.is_alone_sale = 1 AND g.is_delete = 0 " . "LIMIT " . C('related_goods_number');
        $res = $this->query($sql);
        $arr = array();
        foreach ($res as $row) {
            $arr [$row ['goods_id']] ['goods_id'] = $row ['goods_id'];
        ...
        foreach ($res as $tempkey => $value) {
            ...
            $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, p.goods_attr, g.goods_sn, g.goods_name, g.market_price, g.goods_thumb, IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price
					FROM " . $this->pre . "package_goods AS pg
						...
						LEFT JOIN " . $this->pre . "member_price AS mp
							ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]'
					WHERE pg.package_id = " . $value ['act_id'] . " AND g.goods_number != 0 
