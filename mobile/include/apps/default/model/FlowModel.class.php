...
class FlowModel extends BaseModel {
    ...
    function flow_drop_cart_goods($id) {
        ...
        if ($row) {
            ...
            if ($row ['extension_code'] == 'package_buy') {
                $sql = "DELETE FROM " . $this->pre . "cart WHERE user_id='" . $_SESSION['user_id'] . "' " . "AND rec_id = '$id' LIMIT 1";
            }
            ...
            elseif ($row ['parent_id'] == 0 && $row ['is_gift'] == 0) {
                ...
                $sql = "DELETE FROM " . $this->pre . "cart WHERE user_id='" . $_SESSION['user_id'] . "' " . "AND (rec_id IN ($_del_str) OR parent_id = '$row[goods_id]' OR is_gift <> 0)";
            }
            // 如果不是普通商品，只删除该商品即可
            else {
                $sql = "DELETE FROM " . $this->pre . "cart WHERE user_id='" . $_SESSION['user_id'] . "' " . "AND rec_id = '$id' LIMIT 1";
    ...
    function cart_favourable_amount($favourable) {
        /* 查询优惠范围内商品总额的sql */
        $sql = "SELECT SUM(c.goods_price * c.goods_number) as sum " . "FROM " . $this->pre . "cart AS c, " . $this->pre . "goods AS g " . "WHERE c.goods_id = g.goods_id " . "AND c.session_id = '" . SESS_ID . "' " . "AND c.rec_type = '" . CART_GENERAL_GOODS . "' " . "AND c.is_gift = 0 " . "AND c.goods_id > 0 "." AND g.goods_number != 0 ";
    ...
    function add_gift_to_cart($act_id, $id, $price) {
        $sql = "INSERT INTO " . $this->pre . "cart (" . "user_id, session_id, goods_id, goods_sn, goods_name, market_price, goods_price, " . "goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) " . "SELECT '$_SESSION[user_id]', '" . SESS_ID . "', goods_id, goods_sn, goods_name, market_price, " . "'$price', 1, is_real, extension_code, 0, '$act_id', '" . CART_GENERAL_GOODS . "' " . "FROM " . $this->pre . "goods WHERE goods_id = '$id' AND goods_number != 0 ";
        $this->query($sql);
    }
     ...
     * @return integral
     */
    function flow_available_points($cart_id) {
        // $rec_id=implode(" AND c.rec_id= ",$cart_id);
        foreach($cart_id as $k=>$v){
            $rec_id=$v;
            $sql = "SELECT SUM(g.integral * c.goods_number) as sum " . "FROM " . $this->pre . "cart AS c, " . $this->pre . "goods AS g " . "WHERE c.user_id = '" . $_SESSION['user_id'] . "' AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 AND g.goods_number != 0 " . "AND c.rec_type = '" . CART_GENERAL_GOODS . "' AND c.rec_id=".$rec_id;
    ...
    // 增加销量统计
    function adds_touch_goods($flow_type, $extension_code) {
        /* 统计时间段 */
        $period = C('top10_time');
        //近一个月（30天）
        if ($period == 1) { // 一年
     ...
     function add_touch_goods($flow_type, $extension_code) {
        ...
        $arrGoodsid = $this->query($sql);
        foreach ($arrGoodsid as $goodsid) {
            // $sql = "SELECT COUNT(goods_name) AS count FROM ". $this->pre."order_info oi LEFT JOIN ". $this->pre."order_goods og ON oi.order_id = og.order_id WHERE og.goods_id = ".$goodsid['goods_id']." AND oi.supplier_id = 0 AND oi.pay_status = 2";
            $sql = "SELECT sum(goods_number) as sales_count from ".$this->model->pre."order_goods as g left join ".$this->model->pre."order_info as o on o.order_id=g.order_id where g.goods_id =".$goodsid['goods_id'];
            $res = $this->row($sql);
            if($res['sales_count']){
                $sql = "UPDATE ". $this->pre . "touch_goods SET sales_volume = ".$res['sales_count']." WHERE goods_id = ".$goodsid['goods_id'];
            $this->row($sql);
            }
