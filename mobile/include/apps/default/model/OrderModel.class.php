...
class OrderModel extends BaseModel {
    ...
    function order_info($order_id, $order_sn = '') {
        /* 计算订单各种费用之和的语句 */
        $total_fee = " (goods_amount - discount + tax_fee + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee) AS total_fee ";
        ...
        if ($order) {
            ...
            $order['formated_tax'] = price_format($order['tax_fee'], false);
...
// var_dump($order);
    ...
    function order_goods($order_id) {
        ...
        $sql = "SELECT og.rec_id, og.goods_id,og.is_comment, og.goods_name, og.goods_sn, og.market_price, og.goods_number, " .
                "og.goods_price, og.goods_attr, og.is_real, og.parent_id, og.is_gift, " .
                "og.goods_price * og.goods_number AS subtotal, og.extension_code, g.goods_thumb,g.goods_tax " .
    ...
    function cart_goods($type = CART_GENERAL_GOODS) {
       ...
       foreach($rec_id as $k2=>$v2){
            $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number,cart_tax," .
                    "market_price, goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, " .
                    "goods_price * goods_number AS subtotal " .
                    "FROM " . $this->pre .
                    "cart WHERE user_id = '" .$_SESSION['user_id']. "' " .
                    "AND rec_type = '$type' AND rec_id=$v2";//by yangxiaohui
       ...
       $arr=array();
        ...
        foreach ($arr as $key => $value) {
            ...
            // $arr[$key]['formated_cart_tax'] = $value['cart_tax'];
    ...
    function cart_amount($include_gift = true, $type = CART_GENERAL_GOODS) {
       ...
       foreach($rec_id as $k4=>$v4){
            $sql = "SELECT SUM(goods_price * goods_number) " .
                " as amount FROM " . $this->pre .
                "cart WHERE rec_type = '$type' AND rec_id=$v4 ";
    ...
    function cart_weight_price($type = CART_GENERAL_GOODS) {
        ...
        if ($row) {
            ...
            foreach ($row as $val) {
                ...
                $sql = 'SELECT count(*) as count FROM ' .
                        ...
                        $this->pre . 'goods AS g ' .
                        "WHERE g.goods_id = pg.goods_id AND g.goods_number != 0 AND g.is_shipping = 0 AND pg.package_id = '" . $val['goods_id'] . "'";
                $res = $this->row($sql);
                ...
                if ($shipping_count > 0) {
                    ...
                    $sql = 'SELECT SUM(g.goods_weight * pg.goods_number) AS weight, ' .
                            ...
                            $this->pre . 'package_goods AS pg, ' .
                            $this->pre . 'goods AS g ' .
                            "WHERE g.goods_id = pg.goods_id AND g.is_shipping = 0 AND g.goods_number != 0 AND pg.package_id = '" . $val['goods_id'] . "'";
        ...
        $sql = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                "AND rec_type = '$type' AND g.is_shipping = 0 AND g.goods_number != 0 AND c.extension_code != 'package_buy'";
        $row = $this->row($sql);
    ...
    function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0) {
        ...
        /* 取得商品信息 */
        //yang
        $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, g.goods_tax, " .
                ...
                " WHERE g.goods_id = '$goods_id' AND g.goods_number != 0 " .
                " AND g.is_delete = 0";
        $goods = $this->row($sql);
        ...
        if ($num > 0) {
            ...
            $sql = "SELECT goods_number FROM " . $this->pre .
                    "cart WHERE user_id=".$_SESSION['user_id']." AND goods_id = '$goods_id' " .
                    " AND parent_id = 0 AND goods_attr = '" . $this->get_goods_attr_info($spec) . "' " .
                    " AND extension_code <> 'package_buy' " .
    ...
    function clear_cart($type = CART_GENERAL_GOODS,$rec_id) {
        $sql = "DELETE FROM " . $this->pre .
                "cart WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$type' AND rec_id = '$rec_id'";
        $this->query($sql);
    }
    //清空非正常购物的购物车
    function clear_other_cart($type = CART_GENERAL_GOODS) {
        $sql = "DELETE FROM " . $this->pre .
                "cart WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$type'";
        $this->query($sql);
    }
    /**
     * 获得指定的商品属性
    ...
    function get_cart_goods() {
        ...
        foreach ($res as $row) {
            ...
            if (trim($row['goods_attr']) != '') {
                ...
                $sql = "SELECT attr_value FROM " . $this->pre . "goods_attr WHERE goods_attr_id " . db_create_in($row['goods_attr_id']);
        ...
        $total['virtual_goods_count'] = $virtual_goods_count;
            ...
            $sql="SELECT * FROM ".$this->pre."goods_storeroom";
            $storeroom=$this->query($sql);

            
            /*增加代码 商品分仓 by yuanzb  */
            foreach ($goods_list as $key => $cart_good) {
                
                $sql="SELECT goods_storeroom FROM " . $this->pre . "goods WHERE goods_id = " . $cart_good['goods_id'] ." AND goods_number != 0 ";
                $store=$this->query($sql);
                foreach($store as $k=>$v){
                    if(!empty($v['goods_storeroom'])){
                        $goods_list[$key]['goods_storeroom']=$v['goods_storeroom'];
            ...
            foreach ($goods_list as $key => $item) {
                if(!empty($item['goods_storeroom'])){
                    $sql="SELECT store_name FROM " . $this->pre . "goods_storeroom WHERE store_id = " . $item['goods_storeroom'] ;

                }else{
                    $sql="SELECT store_name FROM " . $this->pre . "goods_storeroom WHERE store_id = 1 ";
                }
                

               // $goods_list[$key]['goods_storeroom_name']=$this->query($sql);
                $store_name=$this->query($sql);
                foreach($store_name as $k1=>$v1){
                    $goods_list[$key]['goods_storeroom_name']=$v1['store_name'];
                }
            }
    ...
    function save_consignee_default($address_id) {
        ...
        $sql="UPDATE " . $this->pre .
                "user_address SET is_default = null WHERE user_id = '$_SESSION[user_id]'";
        $a=$this->query($sql);

        $sql="UPDATE " . $this->pre .
                "user_address SET is_default = 1 WHERE address_id = '$address_id'";
        $b=$this->query($sql);

        return $res !== false && $a!==false && $b!==false;
    ...
    function change_order_goods_storage($order_id, $is_dec = true, $storage = 0) {
        ...
        foreach ($res as $row) {
            if ($row['extension_code'] != "package_buy") {
                ...
                $sql = "SELECT is_real" .
                        " FROM " . $this->pre .
                        "goods WHERE goods_id = '" . $row_goods['goods_id'] . "' AND goods_number != 0 ";
    ...
    function integral_to_give($order) {
        ...
        if ($order['extension_code'] == 'group_buy') {
            ...
            $res=$this->row($sql);
            $sql="SELECT integral_money,bonus,discount FROM ".$this->pre."order_info WHERE order_id=".$order['order_id'];
            $order_info=$this->row($sql);

            if($order_info){  // - $total ['bonus'] - $total ['integral_money']-$discount ['discount'];
                $res['custom_points']= floor($res['custom_points']-$order_info['integral_money']-$order_info['bonus']-$order_info['discount']).".00";
            }
            return $res;
        }
    }
