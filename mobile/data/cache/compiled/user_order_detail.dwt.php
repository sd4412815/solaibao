...
<section class="user-order-detail" style="border-top:none;">
  <div class="user-order">
    <p><?php echo $this->_var['lang']['order_status']; ?>：<span><?php echo $this->_var['order']['order_status']; ?></span></p>
...
?>
      <li><a href="<?php echo url('goods/index', array('id'=>$this->_var['goods']['goods_id']));?>" target="_blank"><img src="<?php echo $this->_var['goods']['goods_thumb']; ?>"></a>
        <dl>
          ...
          <dd class="dd-price"><b><?php echo $this->_var['lang']['shop_price']; ?><?php echo $this->_var['goods']['goods_price']; ?> <?php echo $this->_var['lang']['number_to']; ?>：<?php echo $this->_var['goods']['goods_number']; ?></b>
          </dd>
          ...
          <dd class="dd-price"><b class="pull-left">税率：<span class="ect-colory"><?php echo $this->_var['goods']['goods_tax']; ?>%</span></b> 
          </dd>
...
<section class="order-detail-info ect-margin-tb" style="margin-bottom:0;">
  <ul>
    ...
    <li>身份证号码 : <b><?php echo $this->_var['order']['id_card']; ?></b></li>
    <li><?php echo $this->_var['lang']['mobile']; ?>：<b><?php echo $this->_var['order']['mobile']; ?></b></li>
    <?php if ($this->_var['order']['exist_real_goods']): ?>
    <li>收货地址：<b><?php echo $this->_var['order']['province_name']; ?><?php echo $this->_var['order']['city_name']; ?><?php echo $this->_var['order']['district_name']; ?><?php echo $this->_var['order']['address']; ?></b></li>
    <?php endif; ?> 
    <?php if ($this->_var['order']['exist_real_goods']): ?>
    ...
    <li class="text-right">
      ...
      <?php endif; ?> 
       <?php if ($this->_var['order']['tax_fee'] > 0): ?> 
      + 税金:<b class="ect-colory"><?php echo $this->_var['order']['formated_tax']; ?></b><br>
      <?php endif; ?> 
 ...
 <?php if ($this->_var['allow_edit_surplus']): ?>
    <!--<form action="<?php echo url('user/edit_surplus');?>" method="post" name="formFee" id="formFee">
        <section class="order-detail-info ect-margin-tb" style="border-top:none; margin-top:0;">
             <ul style="float:left;">
                <li>
                <b class="pull-left"> <?php echo $this->_var['lang']['use_more_surplus']; ?>：</b><span><input name="surplus" class="ect-padding-lr" type="text" size="6" value="0" /></span>
                <p class="ect-margin-tb"><?php echo $this->_var['max_surplus']; ?></p>
                </li>
             </ul>
             <section class="ect-padding-lr ect-padding-tb ect-margin-tb ect-margin-bottom0" style="float:right; width:8em;">
                <input type="submit" name="Submit" class="btn btn-info ect-btn-info ect-colorf ect-bg" value="确定使用" />
...
<section class="order-detail-info ect-margin-tb  ect-margin-bottom0">
  <ul>
    ...
    <li>
    <form id="payment" name="payment" method="post" action="<?php echo url('user/edit_payment');?>">
...
?>
  ...
  <ul>
   ...
   <!-- <li><?php echo $this->_var['lang']['booking_process']; ?>：<b><?php echo $this->_var['order']['how_oos_name']; ?></b></li> -->
      ...
      <input id="ok_submit" type="submit" name="Submit" class="btn btn-info ect-btn-info ect-colorf ect-bg" value="<?php echo $this->_var['lang']['button_submit']; ?>" style="display:none;" />

    </li>
    <?php endif; ?>
  </ul>
</section>  
<footer class="tihuan"></footer>
<div style="width:100%;">

	<input id="ok" type="button" name="ok_submit" class="btn btn-info ect-margin-tb " style="width:45%; float:left; margin-left:0.6em; margin-top:2.75em;" value="确定" onClick="javascript:fsubmit(document.getElementById('payment'))">

    <?php if ($this->_var['order']['order_amount'] > 0): ?>
    <section class="two-btn ect-padding-tb ect-padding-lr ect-margin-tb text-center" style="width:50%; float: left;" id="zhifubao"><?php echo $this->_var['order']['pay_online']; ?></section><?php else: ?>
    
    <div style="width:100%;" class="two-btn ect-padding-tb ect-padding-lr ect-margin-tb text-center"">
    <?php echo $this->_var['order']['handler']; ?>
    <?php if ($this->_var['order']['invoice_no']): ?>
        <a class="btn btn-info ect-bg-colory" type="button" href="<?php echo url('order_tracking', array('order_id'=>$this->_var['order']['order_id']));?>"><?php echo $this->_var['lang']['order_tracking']; ?></a>
    <?php endif; ?>
    </div>
    <?php endif; ?>
    <?php if ($this->_var['order']['order_amount'] > 0): ?>
    <div style="width:50%; float:right;" class="two-btn ect-padding-tb ect-padding-lr ect-margin-tb text-center""><?php echo $this->_var['order']['handler']; ?>
    <?php if ($this->_var['order']['invoice_no']): ?>
        <a class="btn btn-info ect-bg-colory" type="button" href="<?php echo url('order_tracking', array('order_id'=>$this->_var['order']['order_id']));?>"><?php echo $this->_var['lang']['order_tracking']; ?></a>
    <?php endif; ?>
    </div>
    ...
    <script>
	//yuzhengbo
	var ok_submit = document.getElementById("ok_submit");
	//alert(ok_submit);
	if(ok_submit == null){
			document.getElementById("ok").style.display = "none"
		}
		var user_order = document.getElementById("user_order_hide").innerHTML;
		if(user_order == "查看订单"){
			document.getElementById("user_order_hide").style.display = "none";
        }
