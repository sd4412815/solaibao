...
//    include_once(ROOT_PATH . 'includes/fckeditor/fckeditor.php'); // 包含 html editor 类文件
    ...
    $is_edit = $_REQUEST['act'] == 'edit'; // 点击编辑
    // YWT_增加代码
    if($is_edit){
        $sql = "SELECT goods_tax FROM ". $ecs->table('goods') ."WHERE goods_id = ".$_GET['goods_id'];

        $editres = $db->getRow($sql);
        $smarty->assign('editres',$editres);
    }
    // YWT_增加代码end
...
//    create_html_editor('goods_desc', $goods['goods_desc']);
    create_html_editor('goods_desc', htmlspecialchars($goods['goods_desc'])); /* 修改 by yuanzb 百度编辑器 */
    ...
    /*增加代码 start by yuanzb */
    $sql="SELECT store_id,store_name FROM".$ecs->table('goods_storeroom') . "order by sort_order";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_POST['goods_sn'])
    {
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods') .
                " WHERE goods_sn = '$_POST[goods_sn]' AND supplier_id=0 AND is_delete = 0 AND goods_id <> '$_POST[goods_id]'";
        if ($db->getOne($sql) > 0)
        {
    ...
    $goods_storeroom = isset($_POST['storeroom']) ? $_POST['storeroom'] : 0;/*增加代码 by yuanzb*/
    ...
    $goods_origin = $_POST['product_origin'] ;/*增加代码 by zhuyu */
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_tax, goods_origin,goods_storeroom, goods_name_style, goods_sn, " .///*增加代码 by yuanzb */
                ...
                "VALUES ('$_POST[goods_name]', '$_POST[goods_tax]','$goods_origin', '$goods_storeroom', '$goods_name_style', '$goods_sn', '$catgory_id', " .
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "goods_storeroom = '$goods_storeroom',".
    ...
    /*增加代码 start by yuanzb*/
    // if($is_insert){
    //     $sql = "INSERT INTO " . $ecs->table('products_storeroom') . " (goods_id,store_id, store_number) VALUES ('$goods_id', '$storeroom_id','goods_number')";
    //     $storeroom = $db->query($sql);
    // }
    /*增加代码 end by yuanzb*/
    /* 记录日志 */
    if ($is_insert)
...
//        $link[0] = array('href' => 'goods.php?act=product_list&goods_id=' . $goods_id, 'text' => $_LANG['product']);
        $link[0] = array('href' => 'goods.php?act=product_storeroom&goods_id=' . $goods_id, 'text' => $_LANG['product']);  //这行代码 有修改 by yuanzb
    }
    if ($code == 'virtual_card')
    {
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    if($_REQUEST['stock_warning']==1){
        $goods_list=goods_warn_list($is_delete, ($code=='') ? 1 : 0);
...
elseif ($_REQUEST['act'] == 'product_storeroom')
{
    ...
    $sql_store = "select store_id, store_name from ".$ecs->table("goods_storeroom")." order by sort_order ";
    $res_store = $db->query($sql_store);
    while($row_store = $db->fetchRow($res_store))
    {
        $auto_attr_temp[]=array('goods_attr_id'=>$row_store['store_id'], 'attr_value'=>$row_store['store_name'],  'attr_id'=>'999','attr_name'=>'仓库');
    }
...
elseif ($_REQUEST['act'] == 'product_storeroom_exe')
{
    ...
    $sql_product=" SELECT goods_attr, sum( store_number ) AS product_number FROM ". $ecs->table('products_storeroom'). " WHERE goods_id ='$goods_id' GROUP BY goods_attr";
    $res_product = $db->query($sql_product);
    while ($row_product = $db->fetchRow($res_product))
    {
        $sql_product = "select product_id from ". $ecs->table('products') ." where goods_id='$goods_id' and  goods_attr='$row_product[goods_attr]'  " ;
        $row_product = $db->getOne($sql_product);
        if($row_product)
        {
            $sql="update ".$ecs->table('products')." set product_number='".$row_product['product_number']."' where product_id='$row_product' ";
        }
        else
        {
            $sql="insert into ".$ecs->table('products')."(goods_id, goods_attr, product_sn, product_number ) values('$goods_id', '".$row_product['goods_attr']."', '', '".$row_product['product_number']."' )";
        }
        $db->query($sql);
    }
    ...
    $link[0]['href'] = 'goods.php?act=product_storeroom&goods_id='.$goods_id;
...
/*增加代码 by yuanzb */
//获取税率
elseif ($_REQUEST['act'] == 'get_tax'){
    $catgory_id=$_GET['id'];
    if(!empty($catgory_id)){
        $sql = "SELECT cat_tax FROM " . $ecs->table('category') . " WHERE cat_id = '$catgory_id'";
...
// elseif($_REQUEST['act']=='warn'){
    
    

    // $goods_list = goods_warn_list($_REQUEST['act'] == 'warn' ? 0 : 1, ($_REQUEST['act'] == 'warn') ? (($code == '') ? 1 : 0) : -1);
    // var_dump($goods_list);
    // $smarty->assign('goods_list',   $goods_list['goods']);
    // $smarty->assign('filter',       $goods_list['filter']);
    // $smarty->assign('record_count', $goods_list['record_count']);
...
elseif ($_REQUEST['act'] == 'warn')
{
    ...
    if ($_REQUEST['act'] == 'warn' && isset($handler_list[$code]))
    {
        $smarty->assign('add_handler',      $handler_list[$code]);
    }
    ...
    /* 模板赋值 */
    $goods_ur = array('' => $_LANG['01_goods_warn'], 'virtual_card'=>$_LANG['50_virtual_card_list']);
    $ur_here = 库存警告;
    $smarty->assign('ur_here', $ur_here);

    $action_link = ($_REQUEST['act'] == 'warn') ? add_link($code) : array('href' => 'goods.php?act=warn', 'text' => $_LANG['01_goods_warn']);
    $smarty->assign('action_link',  $action_link);
    $smarty->assign('code',     $code);
    $smarty->assign('cat_list',     cat_list(0, $cat_id));
    $smarty->assign('brand_list',   get_brand_list());
    $smarty->assign('intro_list',   get_intro_list());
    $smarty->assign('lang',         $_LANG);
    $smarty->assign('list_type',    $_REQUEST['act'] == 'warn');
    $smarty->assign('use_storage',  empty($_CFG['use_storage']) ? 0 : 1);
    ...
    $goods_list = goods_warn_list($_REQUEST['act'] == 'warn' ? 0 : 1, ($_REQUEST['act'] == 'warn') ? (($code == '') ? 1 : 0) : -1);
    $goods_list['filter']['stock_warning']=1;
    $smarty->assign('goods_list',   $goods_list['goods']);
    $smarty->assign('filter',       $goods_list['filter']);
    $smarty->assign('record_count', $goods_list['record_count']);
    $smarty->assign('page_count',   $goods_list['page_count']);
    ...
    $htm_file = ($_REQUEST['act'] == 'warn') ?
        'goods_warn.htm' : 'group_list.htm';
    $smarty->display($htm_file);
}
