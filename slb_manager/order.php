...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*,g.goods_tax,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $row['tax_value'] = price_format($row['goods_price'] * $row['goods_number'] * $row['goods_tax'] / 100 );
        $row['formated_subtotal']       = price_format($row['goods_price'] * $row['goods_number'] + ($row['goods_price'] * $row['goods_number'] * $row['goods_tax'] / 100));
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('edit_goods' == $step)
    {
        if (isset($_POST['rec_id']))
        {
            ...
            {
              $sql = "SELECT goods_number goods_tax ".
              ...
              {
                  $sql="SELECT goods_tax FROM ".$GLOBALS['ecs']->table('order_goods')." WHERE rec_id=".$rec_id;
                  $goods_tax=$db->getOne($sql);

                  $tax_fee=$goods_price*$goods_number*($goods_tax/100);
            ...
            $info_tax_fee = get_tax_fee($order_id);
            update_order($order_id, array(
                'goods_amount'  => $goods_amount,
                'tax_fee'       =>$info_tax_fee,
            ));
    ...
    elseif ('money' == $step)
    {
        ...
        $order['tax_fee']       = round(floatval($_POST['tax_fee']), 2);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('goods' == $step)
    {
        ...
        foreach ($goods_list as $key => $value) {
            $goods_list[$key]['total_tax'] = round($value['goods_price']*$value['goods_number']*$value['goods_tax']/100).'.00';
            $goods_list[$key]['subtotals'] = (int)$value['subtotal']+$goods_list[$key]['total_tax'];
            @$a+= $goods_list[$key]['subtotals'];
        }
        $goods_list[$key]['total'] =$a;
        $smarty->assign('goods_list', $goods_list);
        /* 取得商品总金额 */
        $smarty->assign('goods_amount', order_amount($order_id));
        $order['subtotal']      = $order['goods_amount']+$order['tax_fee'];
    }
    ...
    elseif (isset($_POST['print']))
    {
        ...
        foreach ($order_sn_list as $order_sn)
        {
            ...
            while ($row = $db->fetchRow($res))
            {
                ...
                $row['formated_goods_price']    = price_format($row['goods_price']);
                $row['tax_fee']                 =price_format($row['goods_price'] * $row['goods_number']*$row['goods_tax']/100);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time,o.id_card,o.real_name, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
