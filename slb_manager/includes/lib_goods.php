...
function check_goods_sn_exist($goods_sn, $goods_id = 0)
{
    ...
    if (empty($goods_id))
    {
        $sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods') ."
                WHERE goods_sn = '$goods_sn' AND supplier_id=0";
    }
    else
    {
        $sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods') ."
                WHERE goods_sn = '$goods_sn'
                AND goods_id <> '$goods_id' AND supplier_id=0";
    }
...
//获取库存警告的商品
//yang
function goods_warn_list($is_delete, $real_goods=1, $conditions = '')
{
    /* 过滤条件 */
    $param_str = '-' . $is_delete . '-' . $real_goods;
    $result = get_filter($param_str);
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g WHERE is_delete='$is_delete' AND warn_number >= goods_number  $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g WHERE is_delete='$is_delete' AND warn_number >= goods_number $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
...
function check_product_sn_exist($product_sn, $product_id = 0)
{
    ...
    $sql="SELECT goods_id FROM ". $GLOBALS['ecs']->table('goods')."WHERE supplier_id=0 AND goods_sn='$product_sn'";
