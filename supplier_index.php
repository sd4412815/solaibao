...
if (!$smarty->is_cached('mall.dwt', $cache_id))
{
    ...
    $smarty->assign('best_goods',      desc_best(8));   // 精品商品
    $smarty->assign('new_goods',       desc_new(8));     // 最新商品
    $smarty->assign('hot_goods',       desc_hot(8));     // 热门商品

    $smarty->assign('goods_best',      desc_best(1));   // 精品商品
    $smarty->assign('goods_new',       desc_new(1));     // 最新商品
    $smarty->assign('goods_hot',       desc_hot(1));     // 热门商品

    // YWT_增加代码end
    $smarty->assign('category_goods',       get_supplier_category_info());     // 首页推荐分类商品
    $sql = " SELECT supplier_id FROM ". $ecs->table("goods") ." WHERE goods_id = ".$_SESSION['suppId'];
    $goods_lis = $db->getAll($sql);
    
    $smarty->assign('goods_lis',       $goods_lis);    // 首页推荐分类商品
    /* links */
    ...
    if (!empty($cat_recommend_res))
    {
        ...
        $smarty->assign('server_root',ROOT_PATH);
    /* 页面中的动态内容 */
    assign_dynamic('mall');
}
...
function desc_new($limit){
        $sql = "SELECT g.shop_price,g.goods_thumb,g.goods_name,g.market_price,g.goods_id,g.last_update,sgc.parent_id FROM ".$GLOBALS['ecs']->table('goods')."AS g LEFT JOIN ". $GLOBALS['ecs']->table('category') ." AS sgc ON g.cat_id = sgc.cat_id WHERE is_new = 1 AND is_on_sale = 1 AND is_delete = 0 AND goods_number > 0 AND supplier_id = ".$_GET['suppId']." ORDER BY last_update DESC LIMIT ".$limit;
        $descxiaoliang = $GLOBALS['db']->getAll($sql);
        foreach ($descxiaoliang as $key => $value) {
           $descxiaoliang[$key]['suppid'] = $_GET['suppId'];
        }
        return $descxiaoliang;
    }
function desc_hot($limit){
        $sql = "SELECT g.shop_price,g.goods_thumb,g.goods_name,g.market_price,g.goods_id,g.last_update,sgc.parent_id FROM ".$GLOBALS['ecs']->table('goods')."AS g LEFT JOIN ". $GLOBALS['ecs']->table('category') ." AS sgc ON g.cat_id = sgc.cat_id WHERE is_hot = 1 AND is_on_sale = 1 AND is_delete = 0 AND goods_number > 0 AND supplier_id = ".$_GET['suppId']." ORDER BY last_update DESC LIMIT ".$limit;
        $descxiaoliang = $GLOBALS['db']->getAll($sql);
        foreach ($descxiaoliang as $key => $value) {
           $descxiaoliang[$key]['suppid'] = $_GET['suppId'];
        }
...
function desc_best($limit){
        $sql = "SELECT g.shop_price,g.goods_thumb,g.goods_name,g.market_price,g.goods_id,g.last_update,sgc.parent_id FROM ".$GLOBALS['ecs']->table('goods')."AS g LEFT JOIN ". $GLOBALS['ecs']->table('category') ." AS sgc ON g.cat_id = sgc.cat_id WHERE is_best = 1 AND is_on_sale = 1 AND is_delete = 0 AND goods_number > 0 AND supplier_id = ".$_GET['suppId']." ORDER BY last_update DESC LIMIT ".$limit;
        $descxiaoliang = $GLOBALS['db']->getAll($sql);
        foreach ($descxiaoliang as $key => $value) {
           $descxiaoliang[$key]['suppid'] = $_GET['suppId'];
        }
