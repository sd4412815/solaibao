 ...
 * @return  bool         $bool
 */
function register($username, $password, $mobile_phone, $other = array())
{
    /* 检查注册是否关闭 */
    ...
    {
        $GLOBALS['err']->add($GLOBALS['_LANG']['mobile_phone_invalid']);
    }
 ...
 * @return  boolen  $bool
 */
function edit_password($user_id,  $new_password='', $code ='')
{
    if (empty($user_id)) $GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);

    if ($GLOBALS['user']->edit_password($user_id, $new_password, $code))
    {
        return true;
    }
    else
