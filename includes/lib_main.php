...
function update_user_info()
{
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if($row['rank_id']==5){
            $sql='SELECT supplier_id FROM '.$GLOBALS['ecs']->table('supplier').' WHERE user_id='.$_SESSION['user_id'].' AND status=1';
            $supplier_id=$GLOBALS['db']->getRow($sql);
            $_SESSION['supplier_id']=$supplier_id['supplier_id'];
        }else{
 ...
 // $sql1 = 'SELECT last_login FROM ' . $GLOBALS['ecs']->table('users') . " WHERE user_id = '" . $_SESSION['user_id'] . "'";
    // $last_login = $GLOBALS['db']->getOne($sql1);
    // if((int)$last_login < (strtotime(date("y-m-d"))-8*3600))
    // {
    //     $last_login_time=date("Y-m-d H:i:s",$last_login);
    //     $sql2 = " INSERT INTO " . $GLOBALS['ecs']->table('points') . " (user_id,date,detail,pay_points,rank_points)  VALUES(".$_SESSION['user_id'].",'".$last_login_time."','会员每天第一次登录赠送10积分','0','10')";
...
function assign_comment($id, $type, $page = 1)
{
    ...
    $sql = 'SELECT c.*,u.avatar FROM ' . $GLOBALS['ecs']->table('comment') . ' as c left join ' . $GLOBALS['ecs']->table('users') .
        ...
        ' ORDER BY c.comment_id DESC';//会员头像
...
//        $arr[$row['comment_id']]['username'] = $row['user_name'];
        $arr[$row['comment_id']]['username'] = mb_substr($row['user_name'], 0, 1, 'utf-8').'***'. mb_substr($row['user_name'], -1, 1, 'utf-8');
