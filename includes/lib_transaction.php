 ...
 * @return  int         $status         订单状态   10 已完成，1 等待支付，2 等待发货，3 等待收货 ,为空是全部订单
 */
function get_user_orders($user_id, $num = 10, $start=0 , $status=null)
{
    /* 取得订单列表 */
    $arr    = array();
    $where  ='';
/*增加代码 by yuanzb  */
    ...
    $sql = "SELECT order_id, order_sn,consignee, order_status, shipping_status, pay_status, add_time, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount + tax_fee) AS total_fee ".
...
//            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>"; //取消
        }elseif($row['pay_status'] == 2 && ($row['shipping_status'] == 0 ||$row['shipping_status'] == 3) && $row['order_status'] <= 1){
            $row['order_status'] = '等待发货';
            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>"; //取消
        }elseif($row['pay_status'] == 2 && ( $row['shipping_status'] == 5) && ($row['order_status'] == 2||$row['order_status'] == 5)){
            ...
            $row['order_status'] = '部分已发货';
        }elseif($row['order_status'] == 3){
            $row['order_status'] = '无效订单';
        }
...
// 增加代码 by yangxiaohui
//等待支付
function get_user_order_0($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
...
//等待收货
function get_user_order_1($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
        "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
        " FROM " .$GLOBALS['ecs']->table('order_info') .
        " WHERE user_id = '$user_id' AND order_status='1' AND pay_status='2' AND shipping_status='1,3,4' ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
...
//等待发货
function get_user_order_2($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
        "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
        " FROM " .$GLOBALS['ecs']->table('order_info') .
        " WHERE user_id = '$user_id' AND shipping_status='0，3' AND order_status='1' AND pay_status='2' ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
...
function affirm_received($order_id, $user_id = 0)
{
    ...
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id,order_id ,order_sn , order_status,extension_code, shipping_status, pay_status FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    else
    {
        ...
        if ($GLOBALS['db']->query($sql))
        {
            ...
            $sql="SELECT integral FROM" . $GLOBALS['ecs']->table('order_info') . " WHERE order_id='$order_id'";
            $integral=$GLOBALS['db']->getOne($sql);
            $pay_points=$integral;
            /* 计算并发放积分 */
            $integral = integral_to_give1($order);
            log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf('订单 '.$order['order_sn'].'赠送的积分', $order['order_sn']));
            $time=date("Y-m-d H:i:s");

            // if($pay_points)
            // {
            //     $sql = " INSERT INTO " . $GLOBALS['ecs']->table('points') . "(user_id,date,detail,pay_points,rank_points) VALUES(".$_SESSION['user_id'].",'".$time."','订单".$order['order_sn']."赠送的积分','".$pay_points."','0')";
            //     $GLOBALS['db']->query($sql);//TODO 确认收货，增加积分。

            // }
            // 增加代码，增加消费积分 by yangxiaohui
            //增加代码 yang
             $sql = "UPDATE " . $GLOBALS['ecs']->table('order_goods') . " SET shipping_status = '" . SS_RECEIVED . "' WHERE order_id = '$order_id'";
...
//增加代码 by yangxiaohui
//积分分页
function points_pages($user_id, $num = 5, $start = 0)

{
    $arr = array();
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('points'). " WHERE user_id = '$user_id' ORDER BY date DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
 ...
 * @return  int     积分数
 */
function integral_to_give1($order)
{
    ...
    else
    {
        ...
        $sql="SELECT integral_money ,discount FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id=".$order['order_id'];
        $order_info=$GLOBALS['db']->getRow($sql);
        if($order_info){
            $res['custom_points']= floor($res['custom_points']-$order_info['integral_money']-$order_info['discount']).".00";
        }
