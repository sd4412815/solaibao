...
function db_create_in1($item_list, $field_name = '')
{
    ...
    else
    {
        ...
        //YWT增加代码 stat
        if(!empty($_GET['id'])){
            $sql = "SELECT parent_id FROM ". $GLOBALS['ecs']->table('category') ."WHERE cat_id = ".$_GET['id'];
        ...
        if(@$res['parent_id'] == 0){
            ...
            $size_page = ($page)*$size; 
            if(!empty($_GET['id'])){
                $sql = "SELECT cat_id FROM ". $GLOBALS['ecs']->table('category') ."WHERE parent_id = ".$_GET['id'];
                $res = $GLOBALS['db']->getAll($sql);
            }
            if(!empty($res)){
            foreach ($res as $key => $res9) {
            $sql="SELECT g.goods_id,g.goods_thumb,g.goods_name,g.shop_price,g.market_price,g.cat_id,c.parent_id FROM ". $GLOBALS['ecs']->table('goods') ." AS g LEFT JOIN ". $GLOBALS['ecs']->table('category') ." AS c ON g.cat_id = c.cat_id WHERE g.cat_id = ". $res9['cat_id'] ." AND supplier_id = ".$_SESSION['suppId']." AND is_delete = 0 ORDER BY g.shop_price LIMIT ".$size_page. ",".$size;
            $res1[$key] = $GLOBALS['db']->getrow($sql);
            }
           }
        }
         // $item_list_tmp[$key] =$res1;
        // foreach ($res as $key => $value) {
        //    $item_list_tmp[$key] = $value['cat_id'];
...
function check_mobile($mobile){
    $sql="SELECT mobile_phone from".$GLOBALS['ecs']->table('users')." WHERE mobile_phone=".$mobile;
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit,c.cat_tax, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        $arr['no_picture']           = !empty($arr['no_picture']) ? str_replace('../', './', $arr['no_picture']) : 'images/no_picture.png'; // 修改默认商品图片的路径
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        /*增加代码 创建URL by yuanzb 2016-3-28 09:55:31*/
        case 'supplier':
            ...
            break;
        /*增加代码 by yuanzb 2016-3-28 09:55:13*/
        case 'category':
            if (empty($cid))
            {
                return false;
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    ...
    if($pay_points != 0 || $rank_points != 0){
        points_log($user_id,$pay_points,$rank_points,$change_desc);
    }
 ...
 * @param string $change_desc   详细说明
 * @param booler $update        是否更新用户表
 */
function points_log($user_id,  $pay_points = 0,$rank_points = 0, $change_desc = '',$update=false){
    $points_log=array();
    $points_log['user_id']=$user_id;
    $points_log['date']=date('Y-m-d H:i:s',time());
    $points_log['detail']= $change_desc;
    $points_log['pay_points']=$pay_points;
    $points_log['rank_points']=$rank_points;
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('points'), $points_log, 'INSERT');
