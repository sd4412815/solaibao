...
/**
 * 获得指定分类同级的所有分类以及该分类下的子分类2
 *
 * @access  public
 * @param   integer     $cat_id     分类编号
 * @return  array
 */
function get_categories_tree2($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                $cat_arr[$row['cat_id']]['id']   = $row['cat_id'];
                $cat_arr[$row['cat_id']]['name'] = $row['cat_name'];
                $cat_arr[$row['cat_id']]['parent_name'] = @$cat_name;
//                $cat_arr[$row['cat_id']]['url']  = build_uri('supplier', array('cid' => $row['cat_id']), $row['cat_name']);
                $cat_arr[$row['cat_id']]['url']  = "supplier.php?go=category&suppId=".$_SESSION['suppId'] . "&id= ".$row['cat_id'];
                $cat_arr[$row['cat_id']]['supplier_id'] = $_SESSION['suppId'];
                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_tree2($row['cat_id']);
                }
            }
        }
    }
...
function get_categories_tree3($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
        $parent_id = $GLOBALS['db']->getOne($sql);
        $sql = 'SELECT cat_name FROM '. $GLOBALS['ecs']->table('category') . ' WHERE cat_id = '. $parent_id;
        $parent_name = $GLOBALS['db']->getOne($sql);
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $key=> $row)
        {
            ...
            {
                $cat_arr[$row['cat_id']]['id']   = $row['cat_id'];
                $cat_arr[$row['cat_id']]['name'] = @$cat_name;
                $cat_arr[$row['cat_id']]['parent_name'] =@$parent_name;
//                $cat_arr[$row['cat_id']]['url']  = build_uri('supplier', array('cid' => $row['cat_id']), $row['cat_name']);
                $cat_arr[$row['cat_id']]['url']  = "supplier.php?go=category&suppId=".$_SESSION['suppId'] . "&id= ".$row['cat_id'];
                $cat_arr[$row['cat_id']]['supplier_id'] = $_SESSION['suppId'];
                if (isset($row['cat_id']) != NULL)
...
function get_categories_trees($cat_id)
{
        if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
...
//                $cat_arr[$row['cat_id']]['url']  = build_uri('supplier', array('cid' => $row['cat_id']), $row['cat_name']);
                ...
                //supplier.php?go=category&suppId=24&id=48
                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_tree2($row['cat_id']);
                }
...
function get_child_tree2($tree_id = 0)
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            ...
            $three_arr[$row['cat_id']]['name'] = $row['cat_name'];
//            $three_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
            $three_arr[$row['cat_id']]['url']  ="supplier.php?go=category&suppId=".$_SESSION['suppId'] . "&id= ".$row['cat_id'] ;
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order,g.supplier_id ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_on_sale = 1 AND  g.is_alone_sale = 1 AND g.supplier_id = 0 AND g.goods_number!=0 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'.
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.supplier_id, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
...
//    $num = get_library_number("recommend_promotion");
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.supplier_id=0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        /*下面由朱玉添加 2016-03-18 begin*/
        $row['goods_origin']        = $row['goods_origin'];
        $row['goods_storeroom']        = $row['goods_storeroom'];
        /* zhuyu end */
 ...
 */
//yang 获取商品信息
function get_products_info($goods_id)
{
    $return_array = array();
 ...
 * @return  void
 */
function get_goods_sellcount($goods_id){
    $sql = "SELECT *" .
            " FROM " . $GLOBALS['ecs']->table('order_goods') . "WHERE goods_id = '$goods_id'";
    $row = $GLOBALS['db']->getAll($sql);

    return count($row);
}

function get_storeroom_name($goods_storeroom){
    $sql = "SELECT store_name" .
            " FROM " . $GLOBALS['ecs']->table('goods_storeroom') . "WHERE store_id = '$goods_storeroom'";
    $row = $GLOBALS['db']->getRow($sql);
