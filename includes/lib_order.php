...
{
    /* 计算订单各种费用之和的语句 */
    $total_fee = " (goods_amount - discount + tax + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax_fee ) AS total_fee ";
    ...
    if ($order)
    {
        ...
        $order['formated_tax_fee']        = price_format($order['tax_fee'], false);
...
{
    $sql = "SELECT o.tax_fee, o.rec_id, o.goods_id, o.goods_name, o.goods_sn, o.market_price, o.goods_number, " .
            "o.goods_price,o.goods_tax, o.goods_attr, o.is_real, o.parent_id, o.is_gift, " .
 ...
 * @return float  税费
 */
function get_tax_fee($order_id, $include_gift = true){

    $sql = "SELECT SUM(tax_fee) " .
        "FROM " . $GLOBALS['ecs']->table('order_goods') .
        " WHERE order_id = '$order_id'";
    if (!$include_gift)
    {
 ...
 * @return  array
 */
function order_fee($order, $cart_goods, $consignee)
{
    ...
    $total  = array('real_goods_count' => 0,
                    ...
                    'tax'              => 0);
    $weight = 0;
    /* 海关税额 start by yuanzb*/
    foreach ($cart_goods as $key=> $good) {
            $total['tax_fee']   +=  $good['tax_due'];
//            $total['tax_due']+=$good['tax_due']*$good['goods_number'];
    }

    $total['tax_fee_formated']=price_format($total['tax_fee'], false);
    /* 海关税额 end by yuanzb*/
    ...
    /*增加代码 价格 by yuanzb*/
    $total['yuan_goods_price']=$total['goods_price'];
    $total['goods_price']=$total['goods_price'];
//    $total['goods_price']=$total['goods_price']+$total['tax_due'];
    /*增加代码 by yuanzb*/
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $cart_id = explode(".",$_COOKIE['spid']);
            foreach($cart_id as $key1=>$val1){
               
                $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE `user_id` = '" .$_SESSION['user_id']. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND `rec_id`=$val1";
                $count = $GLOBALS['db']->getOne($sql);
                if($count=="1"){
                   $shipping_count+=1;     
                }
            }
            $shipping_count=strval($shipping_count);
    ...
    $total['shipping_id'] = $order['shipping_id'];//yang
    ...
    {   
        ...
        $total['amount'] = $total['goods_price'] - $total['discount'] + $total['tax'] + $total['pack_fee'] + $total['card_fee'] +$total['shipping_fee'] + $total['shipping_insure'] + $total['cod_fee']+$total['tax_fee'];
    ...
    $total['amount_formated']  = price_format($total['amount'], false);//总价
    ...
    {
        $total['will_get_integral'] = get_give_integral1($cart_goods);
    }
    $total['will_get_bonus']        = $order['extension_code'] == 'exchange_goods' ? 0 : price_format(get_total_bonus(), false);

    $total['will_get_integral'] =  floor($total['will_get_integral']-$total['integral_money']- $total['discount']); //yuanzb 修改积分和应付
...
{
    $cart_id = @$_COOKIE['spid'];
    $cart_id = explode(".",$cart_id);
    
    $arr=array();
    foreach ($cart_id as $key => $value) {
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id,c.cart_tax, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, " .
            "c.market_price,c.goods_price, c.goods_attr_id, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " .
            "c.goods_price * c.goods_number AS subtotal " .
            "FROM " . $GLOBALS['ecs']->table('cart') .
            " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods').
            " AS g ON c.goods_id = g.goods_id WHERE rec_id = ".$value. " AND rec_type = " . $type;
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $arr[$key]['tax_due']     = @$arr[$key]['goods_price']*(@$arr[$key]['cart_tax']/100)*@$arr[$key]['goods_number'];
        $arr[$key]['formated_subtotal']     = price_format($value['subtotal']+$arr[$key]['tax_due'], false);
...
/*增加代码 by yuanzb 2016年5月4日10:10:29 */
/**
 * 取得购物车商品
 * @param   int     $type   类型：默认普通商品
 * @return  array   购物车商品数组
 */
function cart_goods1($arr,$type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $item) {
        $sql = "SELECT c.rec_id, c.user_id, c.goods_id,c.cart_tax, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, " .
            ...
            "FROM " . $GLOBALS['ecs']->table('cart') .
            " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods').
            " AS g ON c.goods_id = g.goods_id WHERE rec_id='". $item ."' AND user_id = '" . $_SESSION['user_id'] . "' " .
            "AND rec_type = '$type'";
    ...
    {
        $res[$key]['formated_market_price'] = price_format($value['market_price'], false);
        $res[$key]['formated_goods_price']  = price_format($value['goods_price'], false);
        @$res[$key]['tax_due']     = $res[$key]['goods_price']*($res[$key]['cart_tax']/100)*$res[$key]['goods_number'];
        $res[$key]['formated_subtotal']     = price_format($value['subtotal']+$res[$key]['tax_due'], false);
        if ($value['extension_code'] == 'package_buy')
        {
            $res[$key]['package_goods_list'] = get_package_goods($value['goods_id']);
        }
    }
    return $res;
}
 ...
 * @return  array   购物车商品数组
 */
function cart_goods2($type = CART_GENERAL_GOODS,$card_id)
{
...
function supplier_cart_goods($flow_type,$cart_id)
{
    if(!empty($cart_id)){
       $sql = "SELECT c.rec_id, c.user_id, c.goods_id,c.cart_tax, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number,g.supplier_id, " .
            "c.market_price,c.goods_price, c.goods_attr_id, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " .
            "c.goods_price * c.goods_number AS subtotal " .
            "FROM " . $GLOBALS['ecs']->table('cart') .
            " AS c LEFT JOIN ".$GLOBALS['ecs']->table('goods').
            " AS g ON c.goods_id = g.goods_id WHERE c.user_id = ". $_SESSION['user_id']. " AND rec_type = " . $flow_type." AND supplier_id= ".$_SESSION['suppId'];
            
      $arr = $GLOBALS['db']->getAll($sql);
      foreach ($arr as $key => $value) {
         $goods = (int)$value['goods_price']*(int)$value['goods_number']*(int)$value['cart_tax']/100;//总税金
         $arr[$key]['tax'] = $goods;
         $arr[$key]['all'] = (float)$value['goods_price']*(float)$value['goods_number']+$goods;
         $arr[$key]['all'] = sprintf('%.2f',$arr[$key]['all']);
      }
      // foreach ($arr as $k => $v) {
      //   $goods_price += (int)$v['goods_price']*(int)$v['goods_number'];
      // }
      if(empty($arr)){
                return false;
        }else{
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_name, g.goods_sn,g.goods_tax, g.is_on_sale, g.is_real, ".
    ...
    foreach ($basic_list as $parent_id => $fitting_price)
    {
       ...
       setcookie('xyz',$GLOBALS['db']->insert_id()); 
        /* 改变数量 */
 ...
 */
 //yang
function clear_cart($type = CART_GENERAL_GOODS,$rec_id=null)
{
    ...
    if($rec_id==null){
        ...
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$type' AND rec_id='$rec_id'";
    }

    $GLOBALS['db']->query($sql);
}
 //yang
function clear_cart2($spid)
{
    $spid = explode(".",$spid);
...
function get_cart_goods()
{
    ...
    /* 循环、统计 */
    if($_SESSION['user_id'] == 0){
       $sql = "SELECT c.*,g.is_best,g.is_delete,g.sort_order,g.add_time,g.integral,g.is_alone_sale,g.is_on_sale,g.original_img,g.goods_img,g.goods_thumb,g.goods_desc,g.goods_brief,g.keywords,g.warn_number,g.promote_end_date,g.promote_price,g.shop_price,g.goods_weight,g.provider_name,g.brand_id,g.click_count,g.goods_name_style,g.cat_id,IF(c.parent_id, c.goods_id,g.supplier_id) AS pid " .
            " FROM " . $GLOBALS['ecs']->table('cart') . " AS c " .
        ...
        $sql = "SELECT c.*,g.is_best,g.is_delete,g.sort_order,g.add_time,g.integral,g.is_alone_sale,g.is_on_sale,g.original_img,g.goods_img,g.goods_thumb,g.goods_desc,g.goods_brief,g.keywords,g.warn_number,g.promote_end_date,g.promote_price,g.shop_price,g.goods_weight,g.provider_name,g.brand_id,g.click_count,g.goods_name_style,g.cat_id, IF(c.parent_id, c.goods_id,g.supplier_id) AS pid " .
            ...
            " LEFT JOIN " .$GLOBALS['ecs']->table('goods')." AS g" .
            " ON c.goods_id=g.goods_id" .
            " WHERE g.supplier_id = 0 AND c.user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '" . CART_GENERAL_GOODS . "'" .
            " ORDER BY pid, parent_id " ;
    }
    ...
    $sql="SELECT * FROM " . $GLOBALS['ecs']->table('goods_storeroom') ;
    $storeroom=$GLOBALS['db']->getAll($sql);

    
    /*增加代码 商品分仓 by yuanzb  */
    foreach ($goods_list as $key => $cart_good) {
        $sql = "SELECT goods_storeroom FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = " . $cart_good['goods_id'] ;
        $res = $GLOBALS['db']->getOne($sql);
        if(!empty($res)){
            $goods_list[$key]['goods_storeroom']=$res;
        }else{
            $goods_list[$key]['goods_storeroom']=1;
        }
    }
    foreach ($goods_list as $key => $item) {
        if(!empty($item['goods_storeroom'])){
                $sql="SELECT store_name FROM " . $GLOBALS['ecs']->table('goods_storeroom') . " WHERE store_id = " . $item['goods_storeroom'] ;
            }else{
                $sql="SELECT store_name FROM " . $GLOBALS['ecs']->table('goods_storeroom') . " WHERE store_id = 1";
        }
        $goods_list[$key]['goods_storeroom_name']=$GLOBALS['db']->getOne($sql);
    }
    foreach($goods_list as $k=>$v){
        $result[$v['goods_storeroom']][] = $v;
    }
 ...
 * @return  array
 */
function get_cart_supplier_goods()
{
    ...
    if($_SESSION['user_id'] == 0){
       $sql = "SELECT c.*,g.is_best,g.is_delete,g.sort_order,g.add_time,g.integral,g.is_alone_sale,g.is_on_sale,g.original_img,g.goods_img,g.goods_thumb,g.goods_desc,g.goods_brief,g.keywords,g.warn_number,g.promote_end_date,g.promote_price,g.shop_price,g.goods_weight,g.provider_name,g.brand_id,g.click_count,g.goods_name_style,g.cat_id,IF(c.parent_id, c.goods_id,g.supplier_id) AS pid " .
        ...
        $sql = "SELECT c.*,g.is_best,g.is_delete,g.sort_order,g.add_time,g.integral,g.is_alone_sale,g.is_on_sale,g.original_img,g.goods_img,g.goods_thumb,g.goods_desc,g.goods_brief,g.keywords,g.warn_number,g.promote_end_date,g.promote_price,g.shop_price,g.goods_weight,g.provider_name,g.brand_id,g.click_count,g.goods_name_style,g.cat_id, IF(c.parent_id, c.goods_id,g.supplier_id) AS pid " .
            ...
            " LEFT JOIN " .$GLOBALS['ecs']->table('goods')." AS g" .
            " ON c.goods_id=g.goods_id" .
            " WHERE g.supplier_id = ".$_SESSION['suppId']." AND c.user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '" . CART_GENERAL_GOODS . "'" .
            " ORDER BY pid, parent_id " ;
    }
...
function flow_order_info()
{
    ...
    if (!isset($order['tax_fee']))
    {
        $order['tax_fee'] = 0;  // 初始化税额
    }
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    $sql = "select province from ".$GLOBALS['ecs']->table('order_info')." where order_id='$order_id' ";
    $province = $GLOBALS['db'] ->getOne($sql);
    $sql = "select store_id from ".$GLOBALS['ecs']->table('goods_storeroom')." where  instr(store_province, '#".$province."#') limit 1";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        if ($row['extension_code'] != "package_buy")
        {
            if ($is_dec)
            {
                ...
                change_goods_storage_kufang($row['goods_id'], $row['product_id'], - $row['num'], $store_id);
            }
            else
            {
                change_goods_storage($row['goods_id'], $row['product_id'], $row['num']);
        ...
        else
        {
            ...
            while ($row_goods = $GLOBALS['db']->fetchRow($res_goods))
            {
                ...
                if ($is_dec)
                {
                    ...
                    change_goods_storage_kufang($row_goods['goods_id'], $row['product_id'], - ($row['num'] * $row_goods['goods_number']), $store_id);
                }
...
function change_goods_storage_kufang($good_id, $product_id, $number = 0, $store_id)
{
    ...
    $sql= "UPDATE " . $GLOBALS['ecs']->table('products_storeroom') ."
            ...
            LIMIT 1";
    $products_query_qq2211707  = $GLOBALS['db']->query($sql);
    return  $products;
 ...
 * @return  int     积分数
 */
function get_give_integral($cart_goods=null)
{
...
function get_give_integral1($cart_goods=null)
{
