...
function insert_cart_info()
{
    if($_SESSION['user_id'] != 0){
         $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
    }else{
           $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
    }
   
    // //"SELECT COUNT(*) FROM " . $ecs->table('cart') ." WHERE  user_id = " .$_SESSION['user_id']. " AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type'";
    // $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
 ...
 * @return  string
 */
function insert_cart_info_supplier()
{
    if($_SESSION['user_id'] != 0){
         $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
...
function insert_bought_notes($arr)
{
    ...
    foreach ($bought_notes as $key => $val)
    {
        ...
        $bought_notes[$key]['user_name'] = mb_substr($val['user_name'], 0, 1, 'utf-8').'***'. mb_substr($val['user_name'], -1, 1, 'utf-8');
    }
