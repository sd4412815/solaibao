...
elseif ($_REQUEST['act'] == 'buy')
{
   ...
   //yang
    $product_info = get_products_info($goods['goods_id']);
//    empty($product_info) ? $product_info = array('product_number' => 0, 'product_id' => 0) : '';//数量不足，注释 by yuanzb
    /* 查询：判断指定规格的货品数量是否足够 */
    if($product_info) { //添加判断 by yuanzb
