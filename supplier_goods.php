...
if (!$smarty->is_cached('supplier_goods.dwt', $cache_id))
{
    ...
    else
    {
        if ($goods['brand_id'] > 0)
        {
            ...
            $goods['goods_url'] = "go=goods&suppId=".$goods['supplier_id']."&id=".$goods['goods_id'];
             $sql = "SELECT goods_id,goods_thumb,goods_name,market_price,shop_price FROM " . $ecs->table('goods') .
                    " WHERE supplier_id= " .$goods['supplier_id'];
                    
            $goods['brandid'] = $db->getOne($sql);
            // echo 111;
        }
        // print_r($goods);die;
        ...
        $smarty->assign('categories',         get_categories_trees($goods['cat_id']));  // 分类树
        // $smarty->assign('cates',              get_categories_trees($goods['goods_id']));  // 分类树
        // print_r($goods);die;
        /* meta */
        $smarty->assign('keywords',           htmlspecialchars($goods['keywords']));
        $smarty->assign('description',        htmlspecialchars($goods['goods_brief']));
        $smarty->assign('goods_sell_count',   get_goods_sellcount($goods['goods_id']));//zhuyu 增加2016-3-18 10:29:37
        $smarty->assign('goods_storeroom',    get_storeroom_name($goods['goods_storeroom']));//zhuyu 增加2016-3-18 10:29:37
        ...
        @$hereinfo1 = get_categories_tree3($cat_id);
      
        // p($hereinfo1);
        @$ur_here = "<a href='supplier.php?suppId=".$_GET['suppId']."'>店铺首页</a> <code>&gt;</code> <a href='supplier.php?go=category&suppId=".$_SESSION['suppId']."&id=".$hereinfo1[$cat_id]['id']."'>". $hereinfo1[$cat_id]['parent_name'] ."</a><code>&gt;</code> <a href='".$hereinfo1[$cat_id]['url']."'>".$hereinfo1[$cat_id]['name']."</a><code>&gt;</code> ". $goods['goods_name'];

        $smarty->assign('categories',          get_categories_tree2($goods['cat_id'])); // 分类树  修改代码  YWT
        $smarty->assign('ur_here',             $ur_here);  // 当前位置
        // $smarty->assign('page_title',          $position['title']);                    // 页面标题
        // $smarty->assign('ur_here',             $position['ur_here']);                  // 当前位置

        $properties = get_goods_properties($goods_id);  // 获得商品的规格和属性
//        p(get_goods_gallery($goods_id));
        ...
        $sql="select * from ". $ecs->table('goods_storeroom') ." order by sort_order";
        $res = $db->query($sql);
        $array =array();
        while($row = $db->fetchRow($res))
        {
            ...
            {
                $province = explode("#", $row['store_province']);
                $province_array = array();
                foreach($province as $province)
                {
                    if($province)
                    {
                        $province_array[$province] = $regions[$province];
                    }
                }
            }
            $array[$row['store_name']] =  $province_array ;
        }

        $smarty->assign('province_list', $array);
...
$cart_goods = supplier_cart_goods($flow_type,$cart_id);
        if($cart_goods){
            ...
            @$ass['0']['ass'] +=(int)$value['all'];
        }
    }
         // p($cart_goods);
@$smarty->assign('ass',$ass);       // 当前系统时间
$smarty->display('supplier_goods.dwt',      $cache_id);
...
{
    $sql = 'SELECT COUNT(b.goods_id ) AS num, g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price, g.supplier_id, g.promote_price, g.promote_start_date, g.promote_end_date ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$key]['supplier_id']    = $row['supplier_id'];
        ...
        $arr[$key]['url']         = "go=goods&suppId=".$row['supplier_id']."&id=". $row['goods_id'];
