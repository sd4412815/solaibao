...
$size       = 8;
$record_count = get_cagtegory_goods_count($children, $brand, $min, $max,$size,$page);
...
$pager = get_pager('supplier.php', array('go' => "category&suppId=" . $_SESSION['suppId'] . "&id=".$_GET['id']."&sort=".$sort."&order=".$order), $record_count, $page, 8);  
...
// 排序
 if($_REQUEST['id'] > 0){
    ...
    $sql="SELECT g.goods_id,g.goods_thumb,g.goods_name,g.shop_price,g.market_price,g.cat_id,sgc.parent_id FROM ". $GLOBALS['ecs']->table('goods') ." AS g LEFT JOIN ". $GLOBALS['ecs']->table('category') ." AS sgc ON g.cat_id = sgc.cat_id WHERE ". $children." AND supplier_id = ".$_SESSION['suppId']." AND is_delete = 0 AND is_on_sale = 1 AND goods_number > 0 ORDER BY ".$pager['search']['sort']." ".$pager['search']['order']."  LIMIT ".$size*($page-1)."," . $size;
...
if (!$smarty->is_cached('supplier_category.dwt', $cache_id))
{
    ...
    $hereinfo = get_categories_tree2($cat_id);

    $ur_here = "<a href='supplier.php?suppId=".$_GET['suppId']."'>店铺首页</a> <code>&gt;</code>". $hereinfo[$cat_id]['parent_name'] ."<code>&gt;</code> <a href='".$hereinfo[$cat_id]['url']."'>".$hereinfo[$cat_id]['name']."</a>";

    
    $smarty->assign('ur_here',         $ur_here);  // 当前位置
    $smarty->assign('page_title',      $position['title']);    // 页面标题
    @$smarty->assign('categories',      get_categories_tree2($goods['cat_id'])); // 分类树  修改代码  by yuanzb
    ...
    foreach ($goodslist as $key => $value) {
        $goodslist[$key]['url'] = "supplier.php?go=goods&suppId=".$_SESSION['suppId']."&id=".$goodslist[$key]['goods_id'];
    }
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id,is_show FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");  //修改代码  表名   by yuanzb
}
...
function category_get_goods($children, $brand, $min, $max,  $size, $page, $sort, $order)
{
   ...
   $where .= " AND g.supplier_id =".$_SESSION["suppId"];
    ...
    $sql = 'SELECT distinct g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            "ON mp.goods_id = g.goods_id " . "WHERE $where ORDER BY $sort $order";
 ...
 * @return  integer
 */
function get_cagtegory_goods_count($children, $brand, $min, $max, $size, $page)
{
 //    $where = " g.is_on_sale = 1 AND g.is_alone_sale = 1 AND ".
 ...
 //    }
    ...
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . ' AS sgc LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
                'ON sgc.cat_id = g.cat_id WHERE  is_on_sale = 1 AND is_delete = 0 AND goods_number > 0  AND supplier_id = '.$_SESSION['suppId'].' AND '.$children." LIMIT 0," . $size;
