...
//-- 立即购买添加到购物车
/*------------------------------------------------------ */
elseif ($_REQUEST['step'] == 'add_to_cart2')
{
    ...
    $result = array('error' => 0, 'message' => '', 'content' => '', 'goods_id' => '', 'rec_id' => '');
    $json  = new JSON;
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'id_card'           => empty($_POST['id_card'])        ? '' :   compile_str(make_semiangle(trim($_POST['id_card']))),
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if(@$_GET['rec_id']){
        $cart_id[]=$_GET['rec_id'];
        setCookie('spid',$_GET['rec_id']);
    }else{
        if (empty($_COOKIE['spid'])) {
           show_message('请重试', '', '', 'warning');
        }
    ...
    if(count($cart_id)==1){
        $sql="SELECT is_gift FROM ".$GLOBALS['ecs']->table('cart')." WHERE rec_id=".$cart_id[0];
        $res=$GLOBALS['db']->getOne($sql);

        if($res!=0){
            show_message($_LANG['cart_not_shop'], '', '', 'warning');
        }
    }
    ...
    if (empty($consignee)) {
        show_message('请先填写收货人信息','去填写','flow.php?step=consignee',true);
        exit;
    }
    /* 检查收货人信息是否完整 */
    ...
    // 查看购物车选中的商品是否全为免运费商品，若是则把运费赋为零
    $shipping_count=0;
    foreach($cart_id as $key1=>$val1){
       
        $sql = 'SELECT count(*) FROM ' . $ecs->table('cart') . " WHERE `user_id` = '" .$_SESSION['user_id']. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND `rec_id`=$val1";
        $count = $db->getOne($sql);
        if($count=="1"){
           $shipping_count+=1;     
        }
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    // $cart_goods = cart_goods2($flow_type,$cart_id); // 取得商品列表，计算合计
...
elseif ($_REQUEST['step'] == 'change_integral')
{
    ...
    else
    {
        ...
        /* 对商品信息赋值 */
        $cart_id = explode(".",$_COOKIE['spid']);
        $cart_goods = cart_goods1($cart_id,$flow_type); // 取得商品列表，计算合计
        if (empty($cart_goods))
        {
            $result['error'] = $_LANG['no_goods_in_cart'];
        }
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    /* 检查收货人信息是否完整 */
     ...
     {
//          如果不完整则转向到收货人信息填写界面
         echo "<script>alert('请先填写收货人信息');</script>";
    ...
    if($_COOKIE['spid']){
        ...
        show_message('未选择商品', $_LANG['back_home'], './', 'warning');
    }

    /* 订单中的商品 */
    $cart_goods = cart_goods1($rec_id_arr,$flow_type);//修改获取购物车商品方式 bu yuanzb
    ...
    if(!empty($_POST['id_card'])){
        $consignee['id_card']=$_POST['id_card'];
    }
    /* 收货人信息 */
    if(!empty($consignee)){
    ...
    $order['will_get_integral'] = $total['will_get_integral'];//插入积分 yangxh
    ...
    do
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $order, 'INSERT');//添加数据到order_info   by yuanzb
    ...
    foreach ($cart_id as $k => $v) {
        ...
        $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
            "order_id, goods_id, goods_name, goods_sn,goods_tax,product_id, goods_number, market_price,user_id," .
            "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) " .
            " SELECT '$new_order_id', goods_id, goods_name, goods_sn,cart_tax,product_id, goods_number, market_price, user_id," .
            "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id" .
            " FROM " . $ecs->table('cart') .
            " WHERE user_id = '" . $_SESSION['user_id'] . "' AND rec_type = '$flow_type' AND rec_id='$rec_id'";
        $db->query($sql);
...
/*------------------------------------------------------ */
//-- 更新购物车 && is_array($_POST['goods_number'])
/*------------------------------------------------------ */

elseif ($_REQUEST['step'] == 'update_cart')
{

    $array = explode('.',$_POST['goods_number']);
    $array1 = array($array[0] => $array[1]);
    $_POST['goods_number'] = $array1;
...
{   
    //yang
    $store=$_GET['storeroom'];
   
    $sql="SELECT goods_id FROM ". $GLOBALS['ecs']->table('goods')." WHERE goods_storeroom=".$store;
    $goods_id=$GLOBALS['db']->getAll($sql);

     foreach($goods_id as $k=>$v){
        $sql = "DELETE FROM " . $ecs->table('cart') . " WHERE user_id='" . $_SESSION['user_id'] . "' AND goods_id= ".$v['goods_id']; 
         $db->query($sql);
     }
...
function flow_available_points()
{
    if($_COOKIE['spid']){
        ...
        $arr=str_replace('.',',',$_COOKIE['spid']);

        $where='c.rec_id in ('.$arr.')';

        $sql = "SELECT SUM(g.integral * c.goods_number) ".
            "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, " . $GLOBALS['ecs']->table('goods') . " AS g " .
            "WHERE c.user_id='".$_SESSION['user_id']."' AND ".$where." AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 " .
