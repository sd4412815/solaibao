...
//定义条件中的大范围
$where_big = 'supplier_id='.@$_SESSION['suppliers_id'];
...
//    $password  = md5($_POST['password']);
    ...
    $sql = "INSERT INTO ".$ecs->table('supplier_admin_user')." (user_name,ec_salt,email, password, add_time, nav_list, action_list, role_id, supplier_id, checked) ".
           "VALUES ('".trim($_POST['user_name'])."', '".$ec_salt."','".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id', '$_SESSION[supplier_id]', 1)";
...
/*    if (!empty($admin_name))
 ...
 /*   if (!empty($admin_email))
    ...
    if (!empty($_POST['new_password']))
    {
	    ...
	    {
			$old_ec_password=md5(md5($_POST['password']));
		}
		else
	    {
