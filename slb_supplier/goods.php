...
/*增加代码 by yuanzb 2016-4-1 09:18:37*/
/*------------------------------------------------------ */
//-- 获取税率
/*------------------------------------------------------ */
...
/*增加代码 by yuanzb 2016-4-1 09:18:50*/
/*------------------------------------------------------ */
//-- 商品列表，商品回收站
/*------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $smarty->assign('user_name',$_SESSION['supplier_name']);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        $sql="select * from ". $ecs->table('supplier_goods_cat') ." where goods_id='$goods[goods_id]' ";
        $res = $db->query($sql);
        while ($row = $db->fetchRow($res))
        {
            $cats[]=$row['cat_id'];
    ...
    /*增加代码 by yuanzb*/
//    $sql="SELECT ga.goods_attr_id, ga.attr_id, ga.attr_value FROM ". $GLOBALS['ecs']->table('attribute') . " AS a left join ". $GLOBALS['ecs']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE a.is_attr_gallery=1 and ga.goods_id='" . $GLOBALS['smarty']->_var['goods']['goods_id']. "' order by ga.goods_attr_id ";
//    $color_list=$GLOBALS['db']->getAll($sql);
//    p($color_list);
//    $color_count=count($color_list);
//    $color_list[$color_count]=array('attr_id'=>0, 'attr_value'=>'通用');
//    $GLOBALS['smarty']->assign('color_list', $color_list);
//    $GLOBALS['smarty']->assign('color_count', $color_count+1);
    /*增加代码 by yuanzb*/
...
//    $GLOBALS['smarty']->assign('color_count', $color_count+1);
    ...
    /* 显示商品信息页面 */
    assign_query_info();
    $smarty->display('goods_info_edit.htm');
}
/*------------------------------------------------------ */
//-- 插入商品 更新商品
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name,goods_tax, goods_origin,goods_storeroom, goods_name_style, goods_sn, " .
                ...
                " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$supplier_id', '1')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name,goods_tax, goods_origin,goods_storeroom, goods_name_style, goods_sn, " .
    ...
    if ($is_insert)
    {
        ...
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('supplier_goods_cat') .
            ...
            "VALUES ('$goods_id', '', '$_SESSION[supplier_id]')";
...
/*增加代码 by yuanzb 2016-3-31 11:56:36*/
/*------------------------------------------------------ */
//-- 可分销商品，复制商品
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'distribution'){
    ...
    $smarty->assign('ur_here', '可分销商品');//修改代码 by yuanzb
    ...
    $suppliers_list_count = count($suppliers_list);
    $smarty->assign('suppliers_list', ($suppliers_list_count == 0 ? 0 : $suppliers_list)); // 取供货商列表
    $goods_list = goods_list1( 0, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1);

    //数组排序
    $is_shangjia = array();
    foreach ($goods_list['goods'] as $goods) {
        $is_shangjia[] = $goods['is_shangjia'];
    }
     array_multisort($is_shangjia, SORT_ASC, $goods_list['goods']);
    ...
    $smarty->display('goods_list2.htm');
}

/*------------------------------------------------------ */
//-- 执行复制商品
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'copy2myshop'){
    ...
    //获取商品信息
    $good_id=$_GET['id'];
    $sql="SELECT * FROM " . $GLOBALS['ecs']->table('goods') ." WHERE goods_id= " . $good_id;
    $res=$GLOBALS['db']->getRow($sql);

    //判断商品是否存在
    @$is_existence_sql="SELECT goods_sn FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_sn=" .$res['goods_sn'] ." AND supplier_id= " .$_SESSION[supplier_id];
    $is_existence=$GLOBALS['db']->getRow($is_existence_sql);

    if($is_existence){
        sys_msg( '您已经添加过该商品,请勿重复添加！' , 0, @$link);
        exit();
    }

    $gallery_sql="SELECT * FROM ". $GLOBALS['ecs']->table('goods_gallery')." WHERE goods_id=".$res['goods_id'];
    $gallery_info=$GLOBALS['db']->getRow($gallery_sql);

    unset($res['goods_id']);
    $res['last_update']=time();
    $res['supplier_id']=$_SESSION['supplier_id'];
    $res['supplier_status']=1;
    $res['is_new']=1;
    $res=$db->autoExecute($ecs->table('goods'), $res, 'INSERT');
    $goods_id=$db->insert_id();

    if(!empty($gallery_info)){
        $gallery_info['goods_id']=$goods_id;
        unset($gallery_info['img_id']);
    ...
    if($res){
        $link[0] = array(
            'href' => 'goods.php?act=distribution',
            'text' => '可分销商品列表'
        );
...
elseif($_REQUEST['act'] == 'batch_copy2myshop'){
    ...
    foreach ($_POST['checkboxes'] as $goods) {
        $sql="SELECT * FROM " . $GLOBALS['ecs']->table('goods') ." WHERE goods_id= " . $goods;
        $res=$GLOBALS['db']->getRow($sql);
...
/*增加代码 by yuanzb 2016-3-31 11:56:49*/
...
function get_tree($pid,$arr,$num, $cats){
    global $catstr;
