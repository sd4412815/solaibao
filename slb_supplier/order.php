...
require(dirname(__FILE__) . '/includes/lib_store.php');//yuanzb
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    { 
            $sql = "SELECT discount,goods_amount,tax_fee FROM ". $ecs->table('order_info') ."WHERE order_id = ".$row['order_id'];
            $res1 =  $db->getRow($sql); 
            // p($res1);
        /* 虚拟商品支持 */
        if ($row['is_real'] == 0)
        ...
        $row['formated_subtotal']       = price_format($row['goods_price'] * $row['goods_number']+$row['tax_fee']);
        $row['formated_goods_price']    = price_format($row['goods_price']);
        
        $goods_attr[] = explode(' ', trim($row['goods_attr'])); //将商品属性拆分为一个数组
        $row['formated_subtotal']=trim($row['formated_subtotal'],'￥元');//截取

        $row['allprice'] = $res1['goods_amount'];//所有价格
       
        $row['discount'] = (int)$res1['discount'];//折扣

        foreach ($row as $key => $value) {
            if(is_array($value)){
                 $row['allprice'] += $value['allprice'];//没减折扣前的金额
            }
        ...
        // p($row);
        $row['all_price'] = price_format((int)$row['formated_subtotal']+(int)$row['goods_tax']);//所有价格￥元
        $row['dis_count'] = price_format((int)$res1['discount']);//折扣￥元

        $row['all_prices'] = price_format((int)$row['allprice']);//没减折扣前的价格￥元
        (int)$row['endprices'] = price_format((int)$row['allprice']-(int)$row['discount']);//最终价格￥元
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            ...
            $row['package_goods_list'] = get_package_goods($row['goods_id']);
                ...
                // $goods_list[$k]['tax'] = $goods_list[$k]['goods_price']*$arr[$k]['goods_number']*($arr[$k]['goods_tax']/100);
                // $arr[$k]['goods_amount'] = (float)($arr[$k]['goods_price']*$arr[$k]['goods_number'])+(float)($arr[$k]['tax']);
        $goods_list[] = $row;
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
	...
	/* 代码增加_start  By  yuanzb   发货操作 生成出库单并减库房库存 */
	/*
	foreach ($delivery_stock_result as $value)
        {
			echo $value['goods_id'];
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('shipping' == $step)
    {
		...
		//yuanzb start  只修改运送方式，不计算运费变化与订单总钱的关系
