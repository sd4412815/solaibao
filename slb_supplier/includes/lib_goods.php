...
//		$filter['supplier_status']           = $_REQUEST['supplier_status']!='' ?  $_REQUEST['supplier_status'] : '';
      ...
      /*  if ($filter['supplier_status']!='')
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g WHERE is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where";

        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT g.goods_id, goods_name,g.supplier_id, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
...
//      $filter['supplier_status']           = $_REQUEST['supplier_status']!='' ?  $_REQUEST['supplier_status'] : '';
      ...
      /*  if ($filter['supplier_status']!='')
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g WHERE is_delete='$is_delete' AND g.supplier_id= 0 $where";
        ...
        $sql = "SELECT g.goods_id, goods_name,g.supplier_id, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g WHERE  is_delete='$is_delete' AND g.supplier_id=0 $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
    ...
    foreach ($row as $key => $item) {
        $sql="SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods') ." WHERE goods_sn= " .$item['goods_sn'] ." AND supplier_id= " .$_SESSION['supplier_id'];
        $is_existence=$GLOBALS['db']->getOne($sql);

        if( $is_existence > 0 ){
            $row[$key]['is_shangjia']=1;
        }else{
            $row[$key]['is_shangjia']=0;
        }
    }
...
function goods_list2($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
            " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
            " FROM " . $GLOBALS['ecs']->table('goods') . " AS g WHERE is_delete='$is_delete' $where" ."AND g.supplier_id=0".
            " ORDER BY $filter[sort_by] $filter[sort_order] ".
            " LIMIT " . $filter['start'] . ",$filter[page_size]";
    ...
    ///*增加代码 商品是否以上架 by yuanzb 2016-4-6 11:39:37 */
    foreach ($row as $key => $item) {
        $sql="SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods') ." WHERE goods_sn= " .$item['goods_sn'] ." AND supplier_id= " .$_SESSION['supplier_id'];
        $is_existence=$GLOBALS['db']->getOne($sql);
