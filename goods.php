...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'check_storeroom')
{
    ...
    /* 重新组织 goods_attr 字符串 */
    $attr = !empty($_REQUEST['attr']) ? $_REQUEST['attr'] : "0";
    $sql = "select * from ". $ecs->table("goods_attr") ." where goods_attr_id in ($attr) order by attr_id  ";
    $res =  $db->query($sql);
    $attr="";
    $kkk=0;
    while($row = $db->fetchRow($res))
    {
        ...
        $attr .= $row['goods_attr_id'] ;
        $kkk++;
    }

    $goods_id = (isset($_REQUEST['goods_id'])) ? intval($_REQUEST['goods_id']) : 0;
    $province_id = (isset($_REQUEST['province_id'])) ? intval($_REQUEST['province_id']) : 0;
    $province_name = $db->getOne("select region_name from ". $ecs->table('region') . " where region_id='$province_id' ");
    $store_id = $db->getOne("select store_id  from ". $ecs->table('goods_storeroom') . " where store_province like '%#". $province_id ."#%' ");

    $sql_attr =  $attr ? " and goods_attr='$attr'  " : " ";
    $product_number = $db->getOne("select sum(store_number) as store_number_total from ". $ecs->table('products_storeroom') ."  where goods_id='$goods_id' and store_id='$store_id' $sql_attr ");

    $youhuo = $product_number ? '有货' : '无货';
    $res['result'] = $province_name. '（'. $youhuo . '）';
    $res['store_province_id'] = $province_id;
    die($json->encode($res));
}
